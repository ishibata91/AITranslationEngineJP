...
- 複雑な分岐、依存、責務境界は、必要に応じてチャット本文に Mermaid で示す。
- Mermaid は一時的な説明補助であり、handoff、設計成果物、実装範囲、検証根拠には含めない。
- エージェント間の handoff に Mermaid を含めない。
...
- `外部参照規約` は、skill 外の正本、参照優先順位、衝突時の扱いとして読む。
