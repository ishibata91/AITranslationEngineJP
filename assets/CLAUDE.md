...
- `role-speech-examples.tsv` — 口調の例文。6列（役割区分・種族区分・性別・基底口調セル・英語原文・日本語訳文）で持ち、同じ最高具体度の英日対を入力順ですべて使う。役割語の3キー検索とは分離する。解析は `internal/core/rolespeech`（ParseRoleSpeechExamples）。
