...
description: active exec-plan の task-local design と既存 backend 図から更新対象を特定し、必要なら new component detail 図を判断して、review 用構造差分 D2 / SVG を active exec-plan 配下へ作る。承認後は同じ差分を `diagrams/backend/` 正本へ適用する。
---
...
- active exec-plan の `UI` / `Scenario` / `Logic` と既存 `diagrams/backend/` を読み、どの source 図を更新するかを特定する
...
1. 入力契約を確認し、active exec-plan の `要求要約`、`UI`、`Scenario`、`Logic`、`review 用差分図`、`差分正本適用先` を読む。
...
- 更新対象の特定は、active exec-plan の task-local design と既存 source 図の対応だけで説明できる状態にする
