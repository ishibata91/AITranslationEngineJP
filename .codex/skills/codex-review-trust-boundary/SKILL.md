...
- 差し戻し記録: 判定が 失敗 または 停止 の場合は、出力全文を現行 run の `work_history/runs/<run>/review-reject-trust-boundary.md` に追記する。
...
- 判定が 失敗 または 停止 の場合は、出力全文が `work_history/runs/<run>/review-reject-trust-boundary.md` に追記されている。
