...
3. `distilling-implementation` で facts、constraints、gaps、docs sync 候補を整理する。
4. `planning-implementation` で ordered scope、required reading、validation commands を短い brief にする。
5. `architecting-tests` で failing tests、fixtures、acceptance checks、validation commands を先に固定する。
6. `implementing-frontend` または `implementing-backend` へ handoff して実装する。
7. 実装後は `reviewing-implementation` を 1 回だけ実行する。
