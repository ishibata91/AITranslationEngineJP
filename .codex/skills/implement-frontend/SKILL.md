...
Storybook 確認資源表は次を拘束する。

| 確認資源 | 意味 |
| --- | --- |
| 変更部品 | 今回の frontend 実装で表示または振る舞いが変わった部品 |
| 追加部品 | 今回の frontend 実装で追加した部品 |
| 変更状態 | 今回の frontend 実装で表示または遷移が変わった表示状態 |
| 追加状態 | 今回の frontend 実装で追加した表示状態 |
| story | 変更部品、追加部品、変更状態、追加状態を Storybook で確認する入口 |
