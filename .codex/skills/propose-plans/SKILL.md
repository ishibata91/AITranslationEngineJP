...
- 承認済み design bundle がない限り、`designer` の前に scenario 候補生成器を並列実行する
- scenario 候補生成器は `propose_plans` が直接 spawn し、`designer` に再 spawn させない
...
- scenario candidate coverage の未解決 conflict が残る場合は、design bundle review へ進めず質問票回答待ちにする
...
- allowed: task folder と `plan.md` の作成、更新、closeout、scenario 候補生成器の並列 spawn、独立 agent spawn の packet 作成、人間向け handoff packet 作成、work_reporter への report packet 作成
...
3. distiller と investigator の要否を判定し、承認済み design bundle がない限り scenario 候補生成器と `designer` を使う。
...
5. `propose_plans` が 6 種の scenario 候補生成器を context 継承なしで並列 spawn し、各 generator に同じ template、読む file、禁止事項、期待出力を書く。
6. 候補生成 artifact が 6 件揃わない場合は `designer` に進めず、task folder と不足理由を返して停止する。
7. `designer` を context 継承なしで spawn し、candidate artifact path を packet に入れ、`scenario-design` を必須で作り、詳細要求タイプの未決検出、candidate coverage、conflict 質問票出力を含める。UI 変更がある時だけ `ui-design` を追加する。
8. 必要なら `investigator` を context 継承なしで spawn し、実画面や観測対象を確認する。
9. 戻りを `plan.md` の workflow state に反映する。
10. `needs_human_decision` または未解決 conflict が残る場合は質問票を人間へ返して停止し、0 件になった design bundle だけ human review へ進める。
11. 承認後に `designer` を再度 context 継承なしで spawn し、`implementation-scope` を固定する。
12. 人間が Copilot に渡せる handoff packet を返す。
13. Copilot の修正完了が分かった後、必要なら正本化へ進む。
14. closeout、停止、reroute 時は `work_reporter` へ渡せる benchmark score と completion evidence を整理し、`work_history` へ転記できる report 材料を最後に必ず作る。

## Scenario Candidate Generators

`propose_plans` は `distiller` 結果と task folder を読み、6 体の scenario candidate generator を直接並列 spawn する。
