...
- 拒否条件: 不足 実装対象
- 拒否条件: 不足 承認記録
- 拒否条件: 不足 implementation_scenario_tester 出力 for API test pre-implementation 引き継ぎ
