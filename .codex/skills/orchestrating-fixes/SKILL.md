...
2. Playwright MCP で再現確認できる bug では、先に issue reproduction (`reproduce-issues`) へ handoff し、画面再現を確認させる。観測結果を active plan の `Known Facts` と `Required Evidence` に記録する。
