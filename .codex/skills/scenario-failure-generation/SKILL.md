...
- エージェント実行定義とツール権限は [scenario_failure_generator.toml](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/scenario_failure_generator.toml) の `allowed_write_paths` / `allowed_commands` とする。
...
- 観点名: failure 観点であることを返す。
- 候補 artifact: `docs/exec-plans/active/<task-id>/scenario-candidates.failure.md` を返す。
