...
### UI改善契約レビュー中の agent 継続例外

UI改善契約で task-local UIプロトタイプを作る場合は、`designer` agent を人間UIレビューが終了するまで起動したままにする。
この例外は、UIプロトタイプ確認サーバーを維持し、人間の UI 指摘を `designer` へ直接返すためにだけ使う。
人間の UI 指摘は、`ux_refactor_lane` を経由せず、起動中の `designer` agent が `ui-design.md` と task-local UIプロトタイプへ反映する。
`ux_refactor_lane` は、人間UIレビューの承認、差し戻し、追加質問、終了状態だけを作業計画フォルダに記録する。
人間UIレビューが終了した後は、`designer` agent を閉じ、通常の成果物DAGに戻す。
この例外は、`designer` agent だけに適用し、実装 agent、テスト agent、レビュー agent、`work_reporter` には適用しない。
この例外は、プロダクトコード、プロダクトテスト、docs 正本化、シナリオ設計には使わない。
...
- UI改善契約レビュー中の `designer` agent は、UI改善契約レビュー中の agent 継続例外に従い、人間UIレビュー終了まで閉じない。
...
- 人間UIレビュー中は、task-local UIプロトタイプ確認サーバーが起動したままであり、確認 URL と起動 command が記録されている。
- 人間UIレビュー中は、`designer` agent が起動したままであり、人間の UI 指摘を直接反映できる。
- 人間UIレビュー終了後は、`designer` agent を閉じた状態が記録されている。
- `designer` agent 以外の完了したサブエージェントを閉じた状態が記録されている。
