...
- 返却先は `designer` とする。
- owner artifact は `scenario-lifecycle-generation` の出力規約で固定する。
...
- 必須 artifact: /Users/iorishibata/Repositories/AITranslationEngineJP/.codex/skills/scenario-lifecycle-generation/SKILL.md, /Users/iorishibata/Repositories/AITranslationEngineJP/docs/exec-plans/templates/task-folder/scenario-candidates.viewpoint.md, active task folder または caller-provided task context

## 外部参照規約

- agent runtime と tool policy は [scenario_lifecycle_generator.toml](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/scenario_lifecycle_generator.toml) の `allowed_write_paths` / `allowed_commands` とする。
