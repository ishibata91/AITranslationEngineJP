...
- UI モック本文には、画面上に実際に表示される情報だけを書く。reviewer 向けの設計説明、責務説明、後続工程向けメモを混在させない。
- `責務`、`後続で固める`、`導線成立を優先する`、`placeholder で受ける` のような設計注釈は HTML 内へ書かず、必要なら active exec-plan の `UI モック` section の要点へ短く残す。
- プレースホルダー画面でも、表示内容は `ページ名`、`一文の状態説明`、`戻る/移動導線` までに留める。業務責務の説明や将来実装予定の内訳を書かない。
- モック内の文言、カード、補助テキストは、機能要件をそのまま説明するためではなく、ユーザーが見て理解する画面要素として必要なものだけに絞る。
