...
| セレクタ整合性 | 画面設計と画面設計差分の 画面ID と セレクタ（`aria-label`） が実装と一致しているか |
...
- 画面設計または画面設計差分がある場合は、画面ID と セレクタ（`aria-label`） の実装差分を確認した。
- 画面設計差分がある場合でも、既存の画面設計にある セレクタ（`aria-label`） が壊れていないことを確認した。
