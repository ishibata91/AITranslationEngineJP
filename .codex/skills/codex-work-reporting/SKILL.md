...
description: Codex 側の run 全体レポート作業プロトコル。Codex / Codex implementation レーン ベンチマーク値、レビュー最終状態 YAML、改善ログ から work_history レポート と次回改善事項を残す判断基準を提供する。
---
...
Codex と Codex implementation レーン の ベンチマーク値、レビュー最終状態、改善ログ、検証結果 を同じ run 単位で集約する。
改善可能性の抽出は、ベンチマーク算出スクリプトの出力と `workflow-improvement-log.jsonl` を主材料にする。
work_reporter はベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログだけを前提にする。
...
- ベンチマーク算出結果: run 対象の `analysis/benchmark-score.json` または未作成理由。
...
- ベンチマーク算出スクリプト: [score_transcripts.py](/Users/iorishibata/Repositories/AITranslationEngineJP/scripts/work-history/score_transcripts.py)
...
- `scripts/work-history/score_transcripts.py` の入力引数と生成物
...
hook は使わない。
ベンチマーク算出スクリプトは時間と摩擦の機械指標を出す。
work_reporter はベンチマーク算出スクリプトの出力だけを読む。
スクリプトは改善案、原因推定、責務判断を行わない。

実行コマンド:
- `python3 scripts/work-history/score_transcripts.py --codex-transcript <path> --run-folder work_history/runs/YYYY-MM-DD-<task-id>-run --print-run-folder`
...
`--run-folder` がない場合、フォルダ名は最初の user prompt を安全化して作る。
同名フォルダがある場合は統合する。
...
`user_corrections` は初回依頼後の人間の実メッセージだけを数え、skill 入力内容、下位 agent 通知、承認 レビュー用入力内容 は除外する。

## 判断規約

- `work_reporter` は最後に必ず run 全体レポート を作る。
- 置き場所は `work_history/runs/YYYY-MM-DD-<task-id>-run/` に固定する。
- 改善可能性の抽出は `analysis/benchmark-score.json` と `workflow-improvement-log.jsonl` を主材料にする。
...
- Codex implementation レーン 側の実装事実は、run 内レポート、ベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログ から確認できる範囲だけ転記する。
- ベンチマーク値 欠落、レビュー最終状態 YAML 欠落、改善ログ欠落、ベンチマーク算出スクリプト入力不足 は次回改善 指摘 として扱う。
...
- Codex implementation レーンの事実は run 内レポート、ベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログ からだけ扱う
...
- 根拠参照: レポート生成に使った benchmark、レビュー最終状態、改善ログ、検証結果を返す。
- 不足情報: レポート生成を完了できない不足項目を返す。
- 次判断材料: 人間または `implement_lane` が次を判断できる材料を返す。
- 禁止事項: 出力にツール権限、エージェント実行定義、プロダクトコード変更の指示を含めない。
- レポートパス: README.md、codex.md、analysis/benchmark-score.json、transcript_refs.json、reviewback.*.yaml、workflow-improvement-log.jsonl のパスまたは未作成確認を返す。
...
- Codex レポート summary: run 内レポート、ベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログ から確認できる結果、未完了、重要エラー、検証不足、次に見るべき場所を返す。
- Codex implementation レーン レポート summary: run 内レポート、ベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログ から確認できる完了 引き継ぎ、変更ファイル、検証結果、統合 レビュー 結果、残留リスク、次に見るべき場所を返す。
- run 全体 指摘: 改善すべきこと、時間がかかったこと、無駄だったこと、困ったこと、検証で不足したことを返す。
- benchmark 品質 指摘: ベンチマーク値、レポート、実行定義、スクリプト入力、スクリプト出力 の欠落または破損を次回改善事項として返す。
...
- 改善可能性はベンチマークスクリプト出力と改善ログから抽出した。
- 改善、時間、無駄、困りごとを分けた。
- 人間介入、引き継ぎ、docs 正本化判断を記録対象にした。
- implementation レーンの事実を run 内レポート、ベンチマーク算出スクリプト結果、レビュー最終状態 YAML、改善ログ からだけ扱った。
- 必須根拠として、ベンチマーク値 json または不足理由、レビュー最終状態 YAML または不足理由、改善ログまたは未作成理由、レポート 雛形 paths、利用可能な 検証結果 がある。
...
- run 対象が不足する場合は停止する。
- benchmark、transcript_refs、レビュー最終状態 YAML、改善ログ の有無を確認できない場合は停止する。
