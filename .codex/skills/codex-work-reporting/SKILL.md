...
description: Codex 側の run-wide reporting 作業プロトコル。Codex / Codex implementation lane benchmark score と completion evidence から work_history report と次回改善事項を残す判断基準を提供する。
---
...
Codex workflow の完了、停止、戻し時に、`work_history` へ残す run-wide report 材料を整理する。
...
- 呼び出し元は closeout、停止、戻しを扱う Codex agent とする。
...
- エージェント実行定義とツール権限は [work_reporter.toml](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/work_reporter.toml) の `allowed_write_paths` / `allowed_commands` とする。
...
- report path: README.md、codex.md、codex.md、analysis/benchmark-score.json、transcript_refs.json の path を返す。
- benchmark summary: session count、metrics、scores、evidence refs を分かる範囲で返す。不足は blocker ではなく次回改善事項にする。
- Codex report summary: Codex completion evidence または Codex transcript source_ref から確認できる結果、未完了、重要エラー、検証不足、次に見るべき場所を返す。
- Codex implementation lane report summary: Codex implementation completion evidence または Codex implementation transcript / chat session source_ref から確認できる完了 handoff、変更ファイル、validation result、統合 review 結果、残留リスク、次に見るべき場所を返す。
- run-wide finding: 改善すべきこと、時間がかかったこと、無駄だったこと、困ったこと、検証で不足したことを返す。
- benchmark 品質 finding: benchmark score、evidence、report、runtime、source_ref、transcript の欠落または破損を次回改善事項として返す。
- 次回改善: prompt、handoff、template、benchmark scoring の改善を返す。
- 残留 gap: 未確認、不明、なしを区別して返す。
...
- 必須根拠として、benchmark score json または不足理由、Codex completion evidence または Codex transcript source_ref、Codex implementation completion evidence または Codex implementation transcript / chat session source_ref、report template paths、利用可能な validation result がある。
- 完了判断材料として、work_history/runs/<run>/README.md、codex.md、codex.md が benchmark score と evidence から生成され、次回改善事項が明示されている。
- 残留リスクとして、未確認または不明な gap が返っている。
