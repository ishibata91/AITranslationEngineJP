...
`skill-modification` は、`.codex` 配下の workflow、skill、エージェント実行定義、ツール権限 を変更するための作業プロトコルである。
...
- agent binding と ツール権限 は エージェント実行定義 に置く。
...
- 旧権限 file は残さず、実行権限と write scope は エージェント実行定義の `allowed_write_paths` / `allowed_commands` に置く。
...
- agent を更新する場合は、エージェント実行定義、binding、ツール権限 の所有者を分けて示す。
- 削除対象がある場合は、削除後にどの正本を見ればよいかを示す。
- 出力に プロダクトコード、プロダクトテスト、product 仕様 docs の変更を含めてはいけない。
