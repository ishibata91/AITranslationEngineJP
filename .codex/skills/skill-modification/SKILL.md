...
`skill-modification` は、`.codex` 配下の workflow、skill、agent runtime、tool policy を変更するための作業プロトコルである。
...
- skill / agent / tool policy を変更する場合は、変更対象名とファイルパスを必須にする。
...
- agent binding と tool policy は agent runtime に置く。
...
- agent は実行主体であり、agent runtime と tool policy を持つ。
...
- 責務、tool policy、入力、出力が分かれる場合は、分岐記述ではなく skill または agent の分割を選ぶ。
...
- 旧権限 file は残さず、実行権限と write scope は agent runtime の `allowed_write_paths` / `allowed_commands` に置く。
- workflow 本体 README は個別 skill の `外部参照規約` に入れない。

## 出力規約

- 出力は、更新する `.codex` 配下ファイル、削除するファイル、削除理由、検証方法を含む。
- skill 本体を更新する場合は、`.codex/skill-template.md` の分類順に合わせる。
- agent を更新する場合は、agent runtime、binding、tool policy の所有者を分けて示す。
...
- tool policy と出力義務が skill 側へ混入していないことを確認する。
...
- agent TOML に `allowed_write_paths` / `allowed_commands` がない場合は停止し、不足 key を返す。
- tool policy、agent 所有者、責務境界が不明な場合は停止する。
