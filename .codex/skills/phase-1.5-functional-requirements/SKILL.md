...
description: 第1.5段階の機能要件固定を担当し、active exec-plan の `機能要件` section に in-scope / out-of-scope / open questions を固定する。
---
...
- 第1段階の facts / constraints / gaps を前提に、機能要件として次工程へ渡す境界を固定する
- active exec-plan には `機能要件` section の要約だけを残し、後続の UI モック作成と前段 HITL が読める形にする
- in-scope / out-of-scope / open questions を混在させずに整理する
...
- facts と constraints から機能要件を整理し、未確定事項は `open_questions` として分離する
