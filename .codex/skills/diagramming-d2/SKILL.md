...
6. class 図では生成した `.svg` の縦横比を確認し、`width / height <= 16/9` を満たすまで調整する。
7. validate や render の問題を先に直し、その後にレイアウトと読みやすさを整え、通るまで再実行する。
...
- class 図は情報量を落とさずに `16:9` 以内へ収める。
- class 図で `16:9` を超える時は、まず縦 stack、package 内の 1 列化、label 圧縮、必要時だけ actual name を member や注記へ退避、section 化で幅を減らす。
- class 図で `16:9` を超える時は、detail 図の分割でしか守れない場合に限って主題を保った分割を選ぶ。
- sequence 図の participant はまず日本語の役割名で表し、actual name は区別や検索に必要な場合だけ補助的に添える。
...
- ラベルは短く、意味が明確なものにする。
- 図中ラベルは日本語を既定とし、同じ意味領域では英日混在を避ける。
- 動詞、パラメータ、クラスや型のラベルは、まず日本語の役割名だけで意味が読めるようにする。
- actual name は、同名概念の区別や実装検索に必要な場合だけ補助的に添える。
- 関数名や command 名だけを裸で並べず、何をするものかを論理名で先に読める形にする。
- パラメータや DTO も、値の役割が分かる日本語名を先に置き、actual name は必要時だけ添える。
- `展開する` `固定する` のような抽象的な動詞より、`取り出す` `組み立てる` `渡す` `返す` などの具体語を優先する。
...
- class 図の例:
  ...
  - クラス名はまず日本語の役割名でまとめ、actual name は必要な時だけ member や注記へ退避する。
- ER 図の例:
...
- class 図では目視確認に加え、生成した `.svg` の `viewBox` から `width / height <= 16/9` を確認する。
