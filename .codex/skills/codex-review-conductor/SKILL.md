...
4. 各 agent の score、confidence、findings、evidence、violated invariant、root cause hypothesis、local patch assessment、remediation considerations を受け取る。
...
8. 複数観点の local patch assessment を統合し、primary failure mode、dominant invariant、final exploration scope、minimum durable fix boundary を返す。
9. blocking findings、priority overrides、confidence notes、residual risks、copilot action、remediation handoff を分けて返す。
