...
3. 第1.5段階の機能要件固定として `phase-1.5-functional-requirements` へ handoff し、active plan の `機能要件` section に summary、in-scope、non-functional requirements、out-of-scope、open questions、required reading を固める。
...
18. UI確認として `phase-6.5-ui-check` へ handoff し、Playwright MCP で主要導線、画面状態、承認済み HTML モック artifact に対する視覚構造差分を確認する。handoff には承認済み design bundle と review 用差分図 path を必ず含める。
...
25. 完了前に、第1.6段階で作った UI モック working copy を `docs/mocks/<page-id>/index.html` へ、第2段階の Scenario artifact working copy を `docs/scenario-tests/<topic>.md` へ移す。
26. 完了前に、機能要件・非機能要件を`docs/detail-specs/<page-name>.md`へ移す。
