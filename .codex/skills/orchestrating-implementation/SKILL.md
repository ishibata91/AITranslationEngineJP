...
8. 実装計画として `phase-4-plan` へ handoff し、実装順、担当範囲、検証コマンドを短い implementation brief に落とす。
9. 検証設計として `phase-5-test-implementation` へ handoff し、`Scenario` を tests / fixtures / acceptance checks / validation commands へ適用する。
10. 実装として `phase-6-implement-frontend` または `phase-6-implement-backend` へ handoff する。
11. 実装で設計前提が崩れた時は第2段階へ戻す。実装修正だけで済む時は第6段階に留める。
12. 単体テスト作成として `phase-7-unit-test` へ handoff し、責務と主要分岐を証明する unit test を補う。
13. 実装レビューとして `phase-8-review` を 1 回だけ実行する。
14. review が `reroute` の時は、第6段階へ戻すか、設計差分なら第2段階または第3段階へ戻す。
15. review が `pass` の後に `python3 scripts/harness/run.py --suite all` を実行する。
16. review 用差分図を使った時は承認済み差分を正本へ適用し、review 用差分図を削除する。
17. active plan を `completed/` へ移し、完了結果を記録する。
