...
- `npm run dev:wails:docker-mcp` は起動前に `tmp/logs/wails-dev.log` を削除し、その起動の Wails stdout / stderr だけを同じ path へ書く前提とする
...
- Wails のログは既定では `tmp/logs/wails-dev.log` を MCP で直接確認する
- 初動再現では active fix plan と再現条件を最小入力として着手し、trace plan や logging 結果はある時だけ追加観測に使う
- logging 後の再観測では、追加した観測点に対応する console、`tmp/logs/wails-dev.log`、画面状態を優先して確認する
- 推測を事実として扱わない
- 恒久修正や test 追加を混ぜない
- 再現不能ならその事実を返し、無理に結論を作らない
- UI 確認前に `npm run dev:wails:docker-mcp` が起動済みで、`http://host.docker.internal:34115` を開ける状態と `tmp/logs/wails-dev.log` が当該起動の file として作り直されていることを確認する
