...
- 必須入力: 呼び出し元、引き継ぎ入力、user_instruction_or_task_summary、active_task_folder、設計範囲、レーン担当を受け取る。
- 任意入力: 人間レビュー記録、対象 skill、既存設計成果物、シナリオ候補成果物、既知不足を受け取る。
- 必須成果物: `/Users/iorishibata/Repositories/AITranslationEngineJP/docs/index.md` と 進行中 task folder を受け取る。
