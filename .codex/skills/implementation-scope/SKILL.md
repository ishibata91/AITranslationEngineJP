...
- 端から端までの use case は、ユーザーまたは外部システムの実際の開始入力を模倣する
- UI が入口の機能では、裏側の直接呼び出しや fixture 直接投入だけを完了条件にしない
...
UI が入口の use case では、画面操作、ファイル選択、フォーム入力などのユーザー入力を開始点として扱う。
裏側 API、service、fixture への直接投入は補助検証であり、UI 入口の端から端までの完了判定にはしない。
...
`execution_group` は Copilot 側の ready wave であり、同じ wave 内でも `parallelizable_with` に列挙されない handoff は並列実行しない。
...
- backend contract 確定前の frontend handoff ではない
...
backend と frontend は別 handoff のまま維持し、frontend は backend contract / DTO / gateway 完了後の wave に置く。
final validation、Sonar、Codex review は全 wave 完了後にだけ実行する。

## First Action Rule

handoff 作成時は、各 handoff に `first_action` を必ず書く。
`first_action` は Copilot が最初に閉じる 1 clause だけを示す。
...
7. 各 handoff に owned_scope、depends_on、first_action、validation_commands、completion_signal を書く。
   - validation_commands は validation ownership gate を通し、completion_signal を直接検証するものだけを残す。
   - UI が入口の handoff は、completion_signal にユーザー入力の模倣による確認を含める。
8. 各 handoff に execution_group、ready_wave、parallelizable_with、parallel_blockers を書く。
   - Ready Waves 表で wave ごとの handoff、依存完了条件、並列 pair、blocker を確認できるようにする。
9. 人間が Copilot に渡す entry、禁止事項、期待される完了報告を明示する。
10. Copilot 修正完了後に正本化が必要なら、completion packet の residual risk に残して人間が次 action を判断できるようにする。
...
- UI が入口の handoff は、ユーザー入力の模倣を completion_signal に入れる
...
- Ready Waves 表で Copilot が読む実行順を先に固定する
...
- first_action がない handoff を Copilot に渡さない
