...
- 返却先は `designer` とする。
- owner artifact は `scenario-state-transition-generation` の出力規約で固定する。
...
- 必須 artifact: /Users/iorishibata/Repositories/AITranslationEngineJP/.codex/skills/scenario-state-transition-generation/SKILL.md, /Users/iorishibata/Repositories/AITranslationEngineJP/docs/exec-plans/templates/task-folder/scenario-candidates.viewpoint.md, active task folder または caller-provided task context

## 外部参照規約

- agent runtime と tool policy は [scenario_state_transition_generator.toml](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/scenario_state_transition_generator.toml) の `allowed_write_paths` / `allowed_commands` とする。
...
- 必須出力: viewpoint, candidate_artifact_path, candidate_count, source_requirement_coverage, conflict_hints, human_decision_candidates, missing_source_or_stop_reason
- 出力 field 要件: {"viewpoint": "state-transition を返す", "candidate_artifact_path": "docs/exec-plans/active/<task-id>/scenario-candidates.state-transition.md を返す", "candidate_count": "生成した candidate scenario 数を返す。0 件なら missing_source_or_stop_reason を必ず返す", "source_requirement_coverage": "candidate ごとの source requirement、関連 detail requirement type、観測点を返す", "conflict_hints": "他 viewpoint や最終 scenario 統合時に競合しうる前提、状態、outcome、検証段階を返す", "human_decision_candidates": "AI が確定できない業務判断、状態遷移、外部連携、監査保存対象を返す", "missing_source_or_stop_reason": "必要 source が不足して候補生成できない場合だけ返す"}
...
- completion signal: implement_lane が designer packet に入れる candidate artifact path、候補数、conflict hint、human decision candidate を判断できる
- residual risk key: human_decision_candidates
