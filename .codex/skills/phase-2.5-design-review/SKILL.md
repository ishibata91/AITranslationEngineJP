...
- review は 1 回だけ行う
- 対象は、固定 HTML だけでなく主要なページの動きまである程度再現した UI モック artifact、Scenario テスト一覧 artifact、active exec-plan の `実装計画`、必要時の review 用差分図に限定する
- 要件取りこぼし、責務腐敗、検証不足、構造差分の不整合、主要導線と状態変化の再現不足だけを見る
- 実装改善やコード改善は提案しない
