...
- 観測済み事実 と 仮説 の分離
- 一時観測点の除去
- Chrome DevTools MCP による UI、console message、screenshot 根拠
- 重点 skill の選び方
...
- 根拠 のない結論を固定しない
- Codex implementation レーン のブラウザ操作は Chrome DevTools MCP で行う
- 一時観測点は返却前に除去する
...
- 影響範囲確認: 承認済み実装範囲 外を観測した場合に、今回変更の直接影響を切り分けるための観測範囲、理由、結果を返す。
...
- 観測済み事実 と 仮説 を分けた。
- 承認済み実装範囲 内の 根拠 と、今回変更の直接影響を切り分ける観測範囲確認の根拠だけを扱った。
- 一時変更 と 除去状態 を確認した。
...
- 設計判断が不足している場合は停止する。
- 承認済み実装範囲 外の調査が、今回変更の直接影響を切り分ける観測範囲確認に閉じない場合は停止して `implement_lane` へ返す。
- 承認済み実装範囲 外で恒久修正、プロダクトテスト追加、仕様判断が必要な場合は停止して `implement_lane` へ返す。
