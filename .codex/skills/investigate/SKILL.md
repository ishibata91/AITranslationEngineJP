---
name: investigate
description: Codex 側の設計前調査と探索テスト証跡の作業プロトコル。観測事実、UI 証跡、ログ、未確認事項を根拠 first で扱う判断基準を提供する。
---
# Investigate
...
`investigator` agent が設計前に必要な証拠、または探索テストレーンの探索証跡を集めるための、観測事実、UI 証跡、ログ、仮説、残り 不足 の分け方を提供する。
...
- 非必須調査種別: `investigation_mode` は `再現`、`UI 根拠`、`trace`、`リスク報告`、`探索テスト証跡` のいずれかを受け取る。
