...
標準順序は `要求整理 -> UI モック作成 / Scenario テスト一覧作成 / Logic 実装計画作成 -> 詳細設計 AI review -> 人間承認 -> 検証設計 -> 実装と品質通過 -> UI確認 -> 単体テスト作成 -> 実装レビュー -> 完了` の段階で固定します。
...
- 詳細設計として、`UI モック作成 (phase-2-ui)`、`Scenario テスト一覧作成 (phase-2-scenario)`、`Logic 実装計画作成 (phase-2-logic)` を固める。
- `phase-2-ui`、`phase-2-scenario`、`phase-2-logic` は、オーケストレーターが並列で進める設計対象とする。
- オーケストレーターは、3 つの設計対象の依存関係を見ながら、必要な同期点だけを設けて詳細設計全体をまとめる。
- `UI モック作成 (phase-2-ui)` は、画面実装の正本ではなく、モック HTML を CSS だけで見せるワイヤーフレームとして扱う。画面構造、情報の優先順位、主要な操作の置き場所を固定し、実装都合の component 名や framework 記法は持ち込まない。
- HTML モックは active exec-plan へ埋め込まず、`docs/exec-plans/active/<task-id>.ui.html` に置く。active exec-plan には path と要点だけを残す。
...
- Scenario テスト一覧は active exec-plan へ埋め込まず、`docs/exec-plans/active/<task-id>.scenario.md` に置く。active exec-plan には path と要点だけを残す。
- `Logic 実装計画作成 (phase-2-logic)` は、責務腐敗を避けながら implementation brief を先に固定する工程として扱う。どの task が何を担当し、どの依存解消後に実装へ進むかを `実装計画` として固める。
- `Logic` を図で review したい時は、review 用差分図を作り、追加、削除、移動が読める形で変更意図を表す。
...
- review の対象は HTML モック artifact、Scenario テスト一覧 artifact、active exec-plan の `実装計画`、必要なら review 用差分図を含む詳細設計全体とする。
...
- `UI モック` は `docs/exec-plans/{active|completed}/<task-id>.ui.html` に残し、active exec-plan には path と要点だけを持つ
- `Scenario テスト一覧` は `docs/exec-plans/{active|completed}/<task-id>.scenario.md` に残し、active exec-plan には path と要点だけを持つ
