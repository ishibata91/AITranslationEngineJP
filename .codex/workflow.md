...
- 画面起点で確認できる bug は、事実整理の中で issue reproduction (`reproduce-issues`) を先に使い、Playwright MCP により再現確認する。結果を `Known Facts` と `Required Evidence` に残す。
- 原因分析では、原因仮説を順位付けし、必要な観測だけを最小限で行う。一時観測は恒久修正と混ぜない。
- 一時観測を入れた時、または追加観測が必要な時は issue reproduction (`reproduce-issues`) を再度通し、Playwright console と Wails ログで仮説に対応する観測結果を確認する。
