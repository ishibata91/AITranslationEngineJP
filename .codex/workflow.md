...
- 承認済み作業計画を受け取り、実装順、並列実行単位、依存関係、担当範囲、検証内容を短い実装要約に落とす。
...
- `Implementation Plan` には task ごとの並列可否、依存先、競合しない owned scope を明記する。
- 実装工程では詳細設計を再作成しない。
- この段階の出口成果物は、実装順、並列 task group、依存関係、担当範囲、検証コマンドが固定された実装要約である。
...
- implementation orchestrator (`orchestrating-implementation`) は第4段階で固定した並列 task group と依存関係に従って、独立した task を並列に handoff する。
