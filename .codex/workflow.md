...
- 第1段階で整理した facts / constraints / gaps を前提に、次工程へ渡す機能要件を固定する。
- 実作業では functional requirements phase (`phase-1.5-functional-requirements`) がこの段階を担当する。
- この段階では、対象に含める機能要件、対象外、human 判断が必要な未確定事項を分けて固定する。
- この段階では、UI モック、Scenario テスト一覧、実装計画、review 用差分図はまだ作らない。
- active exec-plan には `機能要件` section を追加し、summary、in-scope、out-of-scope、open questions、required reading を残す。
