...
- 機能要件固定: `functional requirements`
...
標準順序は `要求整理 -> 機能要件固定 -> UI モック作成 -> 人間承認 -> Scenario テスト一覧作成 / Logic 実装計画作成 / review 用構造差分図作成 -> 詳細設計 AI review -> 人間承認 -> 検証設計 -> 実装と品質通過 -> UI確認 -> 単体テスト作成 -> 実装レビュー -> 完了` の段階で固定します。
...
- 第1段階で整理した facts / constraints / gaps を前提に、次工程へ渡す機能要件と非機能要件を固定する。
- 実作業では functional requirements phase (`phase-1.5-functional-requirements`) がこの段階を担当する。
- この段階では、対象に含める機能要件、後続工程が満たすべき非機能要件、対象外、human 判断が必要な未確定事項を分けて固定する。
- この段階では、UI モック、Scenario テスト一覧、実装計画、review 用差分図はまだ作らない。
- active exec-plan には `機能要件` section を追加し、summary、in-scope、non-functional requirements、out-of-scope、open questions、required reading を残す。
- 機能要件が揺れている間は UI モック作成へ渡さない。

### 第1.6段階 UI モック作成

- `UI モック作成 (phase-2-ui)` は前段の機能要件固定後に行い、前段 HITL の承認対象として扱う。
- `phase-2-ui` は actual skill 名を維持し、論理順序だけを前段へ移す。
- 画面構造、情報優先度、主要操作の置き場所を固定し、前段で合意した機能要件を主要導線と状態変化へ写像する。
- task-local の UI モック working copy は `docs/exec-plans/active/<task-id>.ui.html` に置き、active exec-plan には path、最終正本 path、要点だけを残す。
- 機能要件にない振る舞いや詳細設計の都合を UI モックへ持ち込まない。

### 第1.7段階 人間承認

- 前段 HITL として、`機能要件` と `UI モック` を承認対象にする。
- human LGTM は active exec-plan の `機能要件 HITL 状態` と `機能要件 承認記録` に記録する。
- 人間確認が必要な論点が未決のままなら、この段階を通過したとみなさない。
- human review で差し戻しが出た時は、implementation orchestrator (`orchestrating-implementation`) が指摘を `機能要件固定 (phase-1.5-functional-requirements)` または `UI モック作成 (phase-2-ui)` へ切り分けて戻す。
- Scenario テスト一覧、実装計画、review 用差分図はこの段階の承認対象に含めない。

### 第2段階 詳細設計

- 詳細設計として、`Scenario テスト一覧作成 (phase-2-scenario)`、`Logic 実装計画作成 (phase-2-logic)`、`review 用構造差分図作成 (diagramming-structure-diff)` を固める。
- `phase-2-scenario`、`phase-2-logic`、`diagramming-structure-diff` は、オーケストレーターが並列で進める設計対象とする。
...
- この段階の出口成果物は、前段で承認済みの `機能要件` と `UI モック` を前提にした Scenario テスト一覧 artifact、active exec-plan 内の `実装計画`、必要な時だけ作る review 用差分図、差分正本適用先である。
...
- review の対象は、前段で承認済みの `機能要件` と主要なページの動きまである程度再現した UI モック artifact、Scenario テスト一覧 artifact、active exec-plan の `実装計画`、必要なら review 用差分図を含む詳細設計全体とする。
...
- ただし差し戻し理由が機能要件の取りこぼしや前段 UI 合意の崩れである時は、第1.5段階または第1.6段階へ戻し、前段 HITL をやり直す。
- AI review が `pass` でも、人間確認が必要な論点が残る時は、その論点を第3段階へ持ち上げて固定する。
- AI review が `pass` で、人間確認が必要な論点も整理済みの時だけ人間承認に進む。

### 第3段階 人間承認

- human LGTM は作業計画の `詳細設計 承認記録` と `詳細設計 HITL 状態` に記録する。
- 詳細設計と要件の摩擦が残った場合は、人間確認が必要な論点をこの段階で明示し、承認または決定として固定する。
- 実装工程は承認前に起動しない。
- human review で設計差し戻しが出た時は、implementation orchestrator (`orchestrating-implementation`) が指摘を `機能要件固定 (phase-1.5-functional-requirements)`、`UI モック作成 (phase-2-ui)`、`Scenario テスト一覧作成 (phase-2-scenario)`、`Logic 実装計画作成 (phase-2-logic)`、`review 用構造差分図作成 (diagramming-structure-diff)` の修正単位へ切り分け、対応するサブエージェントを起動して artifact と active exec-plan の参照情報を更新させる。
...
- Playwright MCP を使い、HTML モック artifact、Scenario テスト一覧 artifact、受け入れ確認に沿って最小限の操作確認を行う。
...
標準順序は `事実整理 -> 原因分析 -> 再現確認 -> 実装と品質通過 -> UI確認 -> 回帰防止のテスト実装 -> 実装レビュー -> 完了` です。
...
- 画面起点で確認できる bug は、事実整理の中で issue reproduction (`reproduce-issues`) を先に使い、Playwright MCP により再現確認する。`npm run dev:wails:docker-mcp` は起動毎に `tmp/logs/wails-dev.log` を削除して当該起動の Wails stdout / stderr を書き直す前提とし、結果を `Known Facts` と `Required Evidence` に残す。
- 原因分析では、原因仮説を順位付けし、必要な観測だけを最小限で行う。一時観測は恒久修正と混ぜない。
- 一時観測を入れた時、または追加観測が必要な時は issue reproduction (`reproduce-issues`) を再度通し、Playwright console と `tmp/logs/wails-dev.log` で仮説に対応する観測結果を確認する。
- 実装は implementation phase (`phase-6-implement-backend` / `phase-6-implement-frontend`) を再利用し、accepted fix scope、trace 根拠、checks を narrow fix 用 brief として渡す。
- UI確認は UI check phase (`phase-6.5-ui-check`) を第6段階の直後に再利用し、治っていなければ第6段階へ戻す。
- 回帰防止の test 実装は test implementation phase (`phase-5-test-implementation`) を修正後に再利用し、review 前の証明として追加する。
- review は implementation review phase (`phase-8-review`) を再利用し、fix plan、accepted fix scope、validation evidence を基準に単発 review する。
