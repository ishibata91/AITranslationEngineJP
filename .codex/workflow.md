...
標準順序は `directing-implementation -> designing-implementation -> distilling-implementation -> planning-implementation -> architecting-tests -> implementing-frontend or implementing-backend -> sonar-scanner + Sonar CLI open issue gate -> reviewing-implementation -> 4humans sync + close` です。
...
Sonar issue が解消し、`reviewing-implementation` が `pass` なら `4humans sync` と close に進みます。
`reroute` の場合は `directing-implementation` に戻り、同じ lane の中で plan と実装を更新します。

## Fix Lane

標準順序は `directing-fixes -> distilling-fixes -> tracing-fixes -> (必要時 logging-fixes / analyzing-fixes) -> architecting-tests -> implementing-fixes -> reviewing-fixes -> reporting-risks + 4humans sync + close` です。
...
- `pass` なら同じ変更の中で `4humans sync` を完了させて close する
- `reroute` なら direction skill に戻し、plan、tests、実装を必要最小限で更新する
- `4humans sync` は別 lane に押し出さず、その lane の close 条件として扱う
