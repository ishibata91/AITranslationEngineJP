...
- 画面起点で確認できる bug は、事実整理の中で Playwright MCP により先に再現確認し、結果を `Known Facts` と `Required Evidence` に残す。
- 原因分析では、原因仮説を順位付けし、必要な観測だけを最小限で行う。一時観測は恒久修正と混ぜない。
- 一時観測を入れた時は issue reproduction (`reproduce-issues`) を通し、Playwright console と Wails ログで仮説に対応する観測結果を確認する。
