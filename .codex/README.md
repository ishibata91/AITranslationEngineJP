...
- `review_conductor` は Copilot 完了後に人間が `codex exec` で起動する入口として、観点別 review agent を context 継承なしで並列 spawn し、観点別 raw result を保持したまま remediation aggregation に統合する
...
- `work_reporter` は Codex / Copilot benchmark score と completion evidence から `work_history` の run-wide report を生成する。明示 completion evidence が不足する場合は Codex / Copilot transcript または chat session file を source_ref 付き evidence として確認する
- `review_conductor` は Copilot の全 implementation handoff と final validation 完了後、diff から取得した実コードを観点グループ別に score 化し、reviewer result bundle、aggregation trace、primary failure mode、dominant invariant、minimum durable fix boundary を返す
...
14. `review_conductor` は `trust_boundary > behavior > contract > state_invariant` の優先度で conflict を裁定し、観点ごとの raw result を落とさず remediation aggregation に束ね、Copilot が受け取る `copilot_action` を返す
15. 人間が `copilot_action` を Copilot へ戻す。`fix` の場合、Copilot は reviewer result bundle、aggregation trace、remediation handoff から chosen strategy、chosen scope、狭すぎない理由、広げすぎない理由を決め、再実行結果を completion packet に返す
