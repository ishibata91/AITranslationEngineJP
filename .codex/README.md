...
11. Copilot は全 implementation handoff 完了後に suite-all と Sonar check を実行し、その後 `codex exec` で `review_conductor` を呼び出す
12. `review_conductor` は payload または validation 不足なら早期 return し、review 可能なら 4 つの観点別 review agent を並列 spawn する
13. `review_conductor` は `trust_boundary > behavior > contract > state_invariant` の優先度で conflict を裁定し、Copilot が受け取る `copilot_action` を返す
14. Copilot の修正完了と Codex review 結果が分かったら、`propose_plans` が正本化の必要性を判定する
15. 必要なら `propose_plans` が `docs_updater` を context 継承なしで spawn し、承認済み範囲だけを正本化する
16. closeout、停止、reroute 時は `work_reporter` が `codex-work-reporting` を参照し、最後に必ず `work_history` の Codex / Copilot report、benchmark、ラン横断 finding を作る
