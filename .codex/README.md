...
- scenario candidate generator 6 体、`designer`、`investigator`、`docs_updater` は context を引き継がず、handoff packet だけで動く
- `implement_lane` は承認済み execution artifact を実行正本にし、`implementation_investigator`、`implementation_implementer`、`implementation_tester` を context 継承なしで直接 spawn する。final validation 後は観点別 review agent を context 継承なしで並列 spawn し、結果を lossless aggregation に統合する
...
- `implement_lane` は承認済み execution artifact DAG に従い、実装時調査、実装、test、final validation、観点別 review agent の並列 spawn、lossless aggregation、`implementation_action` 分岐を進める
...
- `implement_lane`、`designer`、`investigator`、`docs_updater`、`work_reporter`、review agent は product code と product test を変更しない
...
| `scenario_candidates` | scenario generators | `task_frame` | scenario candidate generators |
...
| `implementation_execution` | `implement_lane` | `implementation_handoff_packet` | `implementation_investigator?`, `implementation_implementer`, `implementation_tester` |
