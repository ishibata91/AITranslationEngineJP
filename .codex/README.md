...
- `review_conductor` は Copilot 完了後に人間が `codex exec` で起動する入口として、観点別 review agent を context 継承なしで並列 spawn する
...
14. `review_conductor` は `trust_boundary > behavior > contract > state_invariant` の優先度で conflict を裁定し、Copilot が受け取る `copilot_action` を返す
15. 人間が `copilot_action` を Copilot へ戻す。`fix`、`rerun_validation`、`rerun_codex_review` の場合だけ、Copilot は指定範囲の再実行結果を completion packet に返す
