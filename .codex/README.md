...
- 実装 orchestration (`implementation-orchestrate`): `skills/implementation-orchestrate/SKILL.md`
...
- `implementation_orchestrator` は `implement_lane` と `fix_lane` の承認済み execution artifact だけを実行正本にし、`implementation_distiller`、`implementation_investigator`、`implementation_implementer`、`implementation_tester` を context 継承なしで spawn して実行し、final validation 後に観点別 review agent を context 継承なしで並列 spawn して結果を lossless aggregation に統合する
- agent は代理人であり、職責、職能、ロール、実行権限の owner として扱う。`agents/<agent>.toml` の中で「自分は何者か」を明示し、permissions、agent 1:1 contract を持つ
...
- agent は自分が何者か、職責、権限、入力、出力、停止条件、reroute 先を自分の runtime 定義内に持つ
...
- `implementation_orchestrator` は `implement_lane` / `fix_lane` の承認済み execution artifact DAG に従い、実装前整理、実装、test、final validation、観点別 review agent の並列 spawn、lossless aggregation、`implementation_action` 分岐を進める
...
- `implementation_orchestrator` は全 implementation handoff と final validation 完了後、diff から取得した実コードを観点グループ別に score 化し、reviewer result bundle、aggregation trace、primary failure mode、dominant invariant、minimum durable fix boundary を completion evidence に残す
...
- implementation lane は docs 正本、`.codex/` workflow 文書、agent contract を変更しない
...
- `refactor_lane`、`exploration_test_lane`、`ux_refactor_lane` は placeholder とし、必須 artifact、gate、actor、completion signal は未定義のままにする
- 各 lane は task-local artifact DAG を持ち、順序は phase 名ではなく `depends_on`、`gate`、`completion_signal` で固定する
- agent は lane そのものではなく、artifact を作る実行主体として扱う
- 全 orchestrator の close 条件には work report と benchmark evidence を必須で含める


## Implement Lane Artifact DAG

新規実装レーンの成果物DAGは次を標準形にする。
順序は `depends_on` と `gate` で固定し、phase 名では固定しない。

| artifact_id | owner | depends_on | gate | completion_signal |
| --- | --- | --- | --- | --- |
| `task_frame` | `implement_lane` | `[]` | user intent known | task id、対象、非対象、成功条件が固定済み |
| `context_distill` | `distiller` | `task_frame` | repo context insufficient | facts、constraints、gaps、required_reading が返却済み |
| `scenario_candidates` | scenario generators | `task_frame`, `context_distill?` | design bundle missing | 6 観点の candidate artifact が揃っている |
| `design_bundle` | `designer` | `scenario_candidates` | human review required | scenario、必要時 UI contract、implementation-scope draft が揃っている |
| `human_design_review` | human | `design_bundle` | HITL hard gate | 承認、差し戻し、追加質問のいずれかが記録済み |
| `implementation_scope` | `designer` | `human_design_review` | human approved | owned_scope、front/back 分割、検証条件が固定済み |
| `implementation_handoff_packet` | `implement_lane` | `implementation_scope` | approved execution artifact only | `implementation_orchestrator` へ渡せる packet が返却済み |
| `implementation_completion` | `implementation_orchestrator` | `implementation_handoff_packet` | implementation executed | implementation result、validation、review aggregation が確認済み |
| `pass_review_evidence` | `implementation_orchestrator` | `implementation_completion` | final validation completed | 観点別 review raw result、aggregation trace、pass / residual、再実行要否が確認済み |
| `canonicalization_decision` | `implement_lane` | `pass_review_evidence` | docs update may be needed | docs 正本化の要否と承認範囲が固定済み |
| `work_report_packet` | `implement_lane` / `work_reporter` | all completed or stopped artifacts | every closeout / stop / reroute | benchmark evidence と report 材料が作成済み |
