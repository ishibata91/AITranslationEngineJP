...
- `refactor_lane` は作業計画フォルダがない場合に `task 枠` として `docs/exec-plans/active/<task-id>/` を作成または確認し、不足している対象範囲を未固定入力として返す。仕様乖離整理起動入力、仕様実装優先判断、構造品質調査起動入力、テスト品質調査起動入力、リファクタ範囲確認、`implementation-scope`、実装実行入力、検証、レビュー、docs 正本化判断を管理する。調査、実装範囲、実装、テスト、ブラウザ確認、レビュー、docs 正本化は担当 agent を起動して委任する。プロダクトコード、プロダクトテスト、docs 正本本文は直接変更しない
...
- `refactor_lane` は task-id または依頼要約から作業計画フォルダを作成または確認してから、仕様乖離整理へ進む。仕様乖離整理起動入力、人間による仕様実装優先判断、構造品質調査起動入力、テスト品質調査起動入力、リファクタ範囲確認、`implementation-scope`、承認済み範囲の修正、テスト、レビュー、docs 正本化判断を扱う
