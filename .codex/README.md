...
- `designer` は `implement_lane` から渡された設計対象と根拠参照を読み、`detail-spec-diff.md` を詳細仕様差分として作る。画面変更がある時は `screen-design-diff.<screen-id>.md` を揃える。人間レビュー後に `implementation-scope` を固定する
...
| `Storybook人間レビュー依頼` | `implement_lane` | `frontend 実装` | なし |
| `frontend 実装後人間レビュー` | human | `Storybook人間レビュー依頼` | human |
