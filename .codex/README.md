...
- Codex agent の人間可読な実行説明は対応する `skills/*/SKILL.md` に置き、binding と tool policy は `agents/<agent>.toml` に置く。agent 1:1 contract がある場合は `agents/references/<agent>/contracts/<agent>.contract.json` に置く
- `.agent.md` は使わない
