...
- `refactor_lane` は仕様乖離整理起動入力、仕様実装優先判断、構造品質調査起動入力、テスト品質調査起動入力、リファクタ範囲確認、`implementation-scope`、実装実行入力、検証、レビュー、docs 正本化判断を管理する。調査、実装範囲、実装、テスト、ブラウザ確認、レビュー、docs 正本化は担当 agent を起動して委任する。プロダクトコード、プロダクトテスト、docs 正本本文は直接変更しない
