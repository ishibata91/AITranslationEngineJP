...
- live Codex agent は workflow orchestrator (`propose_plans`)、design artifact agent (`designer`)、文脈圧縮 agent (`distiller`)、設計前調査 agent (`investigator`)、docs 更新 agent (`docs_updater`)、run report agent (`work_reporter`)、実装後 review conductor (`review_conductor`)、観点別 review agent にする
...
- `work_reporter` は Codex / Copilot telemetry と completion evidence から `work_history` の run-wide report を生成する
...
11. `review_conductor` は payload または validation 不足なら早期 return し、review 可能なら 4 つの観点別 review agent を並列 spawn する
12. `review_conductor` は `trust_boundary > behavior > contract > state_invariant` の優先度で conflict を裁定し、Copilot が受け取る `copilot_action` を返す
13. Copilot の修正完了と Codex review 結果が分かったら、`propose_plans` が正本化の必要性を判定する
14. 必要なら `propose_plans` が `docs_updater` を context 継承なしで spawn し、承認済み範囲だけを正本化する
15. closeout、停止、reroute 時は `work_reporter` が `codex-work-reporting` を参照し、最後に必ず `work_history` の Codex / Copilot report、benchmark、ラン横断 finding を作る
