...
permissions: /Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/references/implementation_tester/permissions.json
contract: /Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/references/implementation_tester/contracts/implementation_tester.contract.json
handoffs:
  - label: Return to orchestrator
    agent: implementation_orchestrator
    prompt: implementation_tester contract の output fields を返す。review が必要なら next step を残す。
    send: false
---
...
この作業は `implementation_tester` agent 定義に基づく。
`single_handoff_packet` と `implementation_tester_context_packet` の owned_scope を証明する product test だけを追加または更新する。
...
active contract は `implementation_tester` に 1 つだけ置く。
...
- primary: `single_handoff_packet`、`implementation_tester_context_packet`、test_subscope、owned_scope、test target
...
正本は [permissions.json](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/references/implementation_tester/permissions.json) とする。
本文には要約だけを書く。

- allowed: owned_scope を証明する product test、fixture、helper の最小変更
- forbidden: docs、`.codex`、`.codex/skills`、`.codex/agents` の変更、paid real AI API 呼び出し
- write scope: product test と必要最小限の test support file

## Contract

正本は [implementation_tester.contract.json](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/references/implementation_tester/contracts/implementation_tester.contract.json) とする。
...
- implementation_tester_context_packet の test_ingredients、test_required_reading、test_validation_entry の順に読む。
- test_subscope が渡された場合は、その sub-scope 内だけを証明し、残りは remaining_test_subscopes に返す。
- `insufficient_context` は structural gate として扱い、implementation_tester_context_packet に behavior_to_prove、public seam、test target、assertion focus、fixture/helper 方針、focused validation が欠ける場合だけ返す。
- `UI人間操作E2E` では開始操作、検証対象の入口、入力模倣方針を必須にする。
- `APIテスト` では public seam、request / response contract、入力開始点、主要観測点を必須にする。
...
1. `single_handoff_packet`、`implementation_tester_context_packet`、test_subscope、test target、owned_scope、handoff 資料のスコープ粒度を確認する。
...
5. unit、APIテスト、UI人間操作E2E のうち最小の証明範囲で test を追加または更新する。
...
- implementation_tester_context_packet、scenario artifact、test target、owned_scope が不足している。
...
- handoff 先: `implementation_orchestrator`
- 渡す contract: [implementation_tester.contract.json](/Users/iorishibata/Repositories/AITranslationEngineJP/.codex/agents/references/implementation_tester/contracts/implementation_tester.contract.json)
