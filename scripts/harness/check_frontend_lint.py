...
def main() -> int:
    parser = build_parser("Run the frontend lint harness.", default_repo_root(__file__))
    ...
    if not package_json_path.exists():
        ...
        return finalize_failures("Frontend lint harness", 0)

    package = load_json(package_json_path)
    if not has_script(package, "lint:frontend"):
        report_skip(f"SKIP no lint:frontend script in {package_json_path}")
        return finalize_failures("Frontend lint harness", 0)

    package_manager = resolve_package_manager(repo_root)
    failures = invoke_step(package_manager, ["run", "lint:frontend"], repo_root)
    return finalize_failures("Frontend lint harness", failures)
