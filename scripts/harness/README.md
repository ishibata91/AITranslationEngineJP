...
- `backend-lint`: repo root package の `lint:backend` を入口にして Rust lint だけを実行する
...
- repo root の `gate:execution` は `format:check`、`lint`、`scan:sonar`、`test`、`src-tauri/` の Rust test、`build` をこの順でまとめる
