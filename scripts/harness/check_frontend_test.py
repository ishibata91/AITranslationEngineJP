...
def main() -> int:
    parser = build_parser("Run the frontend test harness.", default_repo_root(__file__))
    ...
    if not package_json_path.exists():
        ...
        return finalize_failures("Frontend test harness", 0)

    package = load_json(package_json_path)
    if not has_script(package, "test:frontend"):
        report_skip(f"SKIP no test:frontend script in {package_json_path}")
        return finalize_failures("Frontend test harness", 0)

    package_manager = resolve_package_manager(repo_root)
    failures = invoke_step(package_manager, ["run", "test:frontend"], repo_root)
    return finalize_failures("Frontend test harness", failures)
