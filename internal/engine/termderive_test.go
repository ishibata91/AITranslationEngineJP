...
func TestDeriveTerms(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:      "two は base ゲームの空白2語姓名を中黒2語で整列して採る",
			fullPairs: []NamePair{{Source: "Mercer Frey", Dest: "メルセル・フレイ", BaseGame: true}},
			usage:     empty,
			want:      []string{"Mercer=>メルセル:two", "Frey=>フレイ:two"},
		},
		{
			name:      "two は base ゲーム以外では派生しない",
			fullPairs: []NamePair{{Source: "Mercer Frey", Dest: "メルセル・フレイ", BaseGame: false}},
			usage:     empty,
			want:      nil,
		},
		{
			name:      "two は英語と中黒の語数が一致しなければ捨てる",
			fullPairs: []NamePair{{Source: "Mercer Frey Extra", Dest: "メルセル・フレイ", BaseGame: true}},
			usage:     empty,
			want:      nil,
		},
		{
			name:      "two は Dest に漢字（役割語）を含めば捨てる",
			fullPairs: []NamePair{{Source: "Foobar Guard", Dest: "フーバー・衛兵", BaseGame: true}},
			usage:     empty,
			want:      nil,
		},
		{
			name:      "two は語単位で安全フィルタを通った語だけ採る",
			fullPairs: []NamePair{{Source: "Ab Frey", Dest: "エ・フレイ", BaseGame: true}},
			usage:     empty,
