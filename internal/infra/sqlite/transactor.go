...
// SQLiteTransactor は repository.Transactor の SQLite 実装。
type SQLiteTransactor struct {
	db *sqlx.DB
}

// NewSQLiteTransactor は repository.Transactor を返す。
func NewSQLiteTransactor(db *sqlx.DB) repository.Transactor {
...
func (t *SQLiteTransactor) WithTransaction(ctx context.Context, fn func(context.Context) error) error {
