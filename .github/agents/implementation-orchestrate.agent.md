...
- 人間から Codex review の戻り値が渡された場合だけ、`copilot_action` で受け取り、`fix` では remediation handoff から chosen strategy と chosen scope を説明する。
...
24. 人間から `copilot_action: fix` が戻された場合は、`remediation_handoff` から chosen_strategy、chosen_scope、why_this_scope、why_not_narrower、why_not_wider、planned_changes、invariant_tests を作り、修正後に final validation と Codex review request payload を再作成する。
