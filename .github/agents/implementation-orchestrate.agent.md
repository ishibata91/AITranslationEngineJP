...
- completion packet には、Codex 側 `work_reporter` が読む implementation evidence と telemetry events を必ず含める。
...
30. 最後に必ず completion evidence と telemetry events を返し、completed_handoffs、touched_files、validation、residual、narrowing、blocked reason、人間が次に見るべき場所を含める。
