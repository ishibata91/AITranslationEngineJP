...
`single_handoff_packet` と `lane_context_packet` の owned_scope を証明する product test だけを追加または更新する。
...
- primary: `single_handoff_packet`、`lane_context_packet`、owned_scope、test target
...
1. `single_handoff_packet`、`lane_context_packet`、test target、owned_scope、handoff 資料のスコープ粒度を確認する。
