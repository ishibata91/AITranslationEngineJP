...
description: GitHub Copilot 側の backend 実装知識 package。layer 責務、validation、Sonar gate の判断基準を提供する。
---
...
- validation と error path を実装する時
- Sonar 件数 gate を確認する時
...
- touched package の test を優先する
- backend を含む場合は Sonar critical gate を返す

## DO / DON'T

DO:
- usecase / service / repository / adapter の責務を確認する
- touched package の tests を実行する
- Sonar gate の結果を closeout に残す
