...
description: GitHub Copilot 側の実装前文脈整理の共通知識 package。承認済み implementation-scope から実装に必要な facts を圧縮する判断基準を提供する。
---
...
`implementation-distiller` agent が、承認済み `implementation-scope` の handoff 1 件から implementation context packet を作る時の共通判断を提供する。
...
- implementation-scope と owned_scope の対応づけ
- focused skill の選び方
