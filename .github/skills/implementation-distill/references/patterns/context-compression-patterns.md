...
`implementation-distiller` が single_handoff_packet 1 件を実装可能な lane_context_packet へ圧縮するための判断パターンをまとめる。
...
- 要件、実装方針、決定事項は source と implementer impact 付きで要約する。
...
- required_reading の先頭は、implementer が最初に触る実 code の path、symbol、line number にする。
...
- 類似 context を distracting_context に分けず、implementer の読む対象に混ぜている。
...
- implementer がどの file のどの関数を変更すべきか再調査する必要がある。
