...
- 要件、実装方針、決定事項は implementer が再読不要な粒度で要約する
...
- implementer が architecture 正本を全文再読しなくてよいよう、今回の handoff に効く境界だけを `requirements_policy_decisions` へ圧縮する
...
- requirements_policy_decisions に implementer impact を残す
...
- `docs/architecture.md` や `docs/lint-policy.md` の全文読解を implementer へ丸投げしない
