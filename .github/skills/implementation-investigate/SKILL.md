...
description: GitHub Copilot 側の実装時調査の共通知識 package。承認済み implementation-scope 内で evidence first に調査する判断基準を提供する。
---
...
`investigator` agent が、承認済み `implementation-scope` と owned_scope 内で実装時の証拠を集める時の共通判断を提供する。
