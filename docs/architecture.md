...
- fix-batch-failure-handling（2026-08-02）で OpenAI と xAI の各進行段を最大1000件の外部 batch に分け、`engine` は現在の外部 batch が完了してから同じ進行段の未送信分を次の外部 batch へ送る形にした。`batch_request` の `custom_id` が同じ進行段で送信済みの対象を識別し、`batch_translation` の各外部 batch ID は現在処理中の1件を指す。OpenAI の `status=failed` は結果取得可能な終端として扱わず、外部 batch ID と失敗理由を既存の画面エラー経路へ返し、結果取得、次の送信、進行段更新を止める。`completed`、`expired`、`cancelled` の既存動作は維持する。
- batch-auto-polling（2026-08-02）で OpenAI と xAI の batch 翻訳の状態確認を翻訳実行画面の frontend が持つ形にした。利用者がバッチ実行を押した後、画面が開いている間は前回の呼び出しが完了してから10秒後に次の状態確認を1件だけ行う。現在の外部 batch が取り込み可能なら、既存の backend 処理を1回進め、同じ進行段の次チャンク、固有名段から本文段、完了まで自動で進む。画面を閉じると次回の状態確認を取り消し、再表示だけでは再開しない。利用者がバッチ実行を押した時に保存済みの進行を確認し、進行中なら新しい外部 batch を送らずに状態確認を再開する。API キーと endpoint は画面表示中の接続にだけ使い、永続化しない。
