...
- `api`: Wails Bind の公開面。辞書・ルール・設定の CRUD を `store` へ素通しし、用語辞書編集は翻訳用の読み取り経路と独立した書込経路へ渡す。翻訳ジョブを `engine` の goroutine として起動し、進捗を runtime events で push する。
...
- `store`: 中心SQLiteへの薄いデータアクセスと、`db/dictionary.sqlite3` の事前作成済み翻訳辞書readerおよび用語辞書編集用の書込経路を持つ。翻訳辞書readerは読み取り専用で開き、本文翻訳の参考語を返す。用語辞書の書込経路は翻訳用readerとは別接続で開く。sqlx を使い、entity ごとの repository interface は作らず関数で持つ。プロンプトテンプレート（base 指示・口調指示の雛形）の CRUD を含む。残存の keyring secret store を secret 子に置く。
...
- `bootstrap`: composition root。中心 store、翻訳用reader、用語辞書編集用writer、`provider` を生成し、`engine` と `api` へ注入する唯一の場所。
