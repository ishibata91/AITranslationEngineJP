...
- modの翻訳前に､事前に任意のプラグインのNPCのNPCペルソナ生成フェーズを実行し､マスターペルソナとして構築可能なこと｡
- クエストの進行情報を元に､翻訳補助メタデータとしてクエスト補助メタデータを提供できること｡
- 既存の用語辞書(xml)を元に､一貫した単語訳でAIに翻訳させられること｡
- 事前にxTranslator形式の翻訳ファイルを取り込み､マスター辞書として構築可能なこと｡
- マスター辞書として構築済み、または用語翻訳で翻訳済みの対象は翻訳対象とせず、置き換えること。
...
- 基盤構築フローでは、任意のプラグインの NPC 由来のマスターペルソナと、xTranslator 取り込み済みのマスター辞書を構築する
...
- 「ペルソナ」は個別 NPC の情報を指し、「マスターペルソナ」は任意のプラグインの由来で基準化された集合を指す。
