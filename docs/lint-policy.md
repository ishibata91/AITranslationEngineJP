...
- `SonarScanner + Docker MCP SonarQube`: `TS` / `Svelte` / `Rust` を対象に、code smell、complexity、security hotspot などの server-side issue gate を担当する
...
初期 gate の責務は `Oxlint` / `ESLint` / `Knip` / `rustfmt check` / `SonarScanner + Docker MCP SonarQube` / `clippy` に固定する。
...
- `SonarScanner + Docker MCP SonarQube` は既存の `Oxlint` / `ESLint` / `Knip` / `clippy` を置き換えず、構造解析系 lint で取り切れない issue を補完する追加層として gate に入れる
...
- implementation lane は `docker mcp tools call search_sonar_issues_in_projects --gateway-arg=--profile --gateway-arg=codexmcps` を使う helper script 経由で issue を読み、`project == ishibata91_AITranslationEngineJP` かつ `status == OPEN` の issue が残る限り implementing skill に差し戻す
- import graph や cycle の主担当は Sonar に移さず、専用 lint / 静的解析に残す
- project context は `sonar-project.properties` の project key を正本とし、Docker MCP Sonar query でも同じ key を使う
- Sonar issue read に必要な認証と組織設定は Sonar CLI ではなく `codexmcps` profile の `mcp/sonarqube` secret / config を正本とする
...
- scanner 実行後は implementation lane が `python3 .codex/skills/directing-implementation/scripts/get-open-sonar-issues.py --project ishibata91_AITranslationEngineJP` を使って `project == ishibata91_AITranslationEngineJP` かつ `status == OPEN` issue を取得する
