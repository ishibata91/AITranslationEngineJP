...
- `SonarScanner + Sonar MCP`: `TS` / `Svelte` / `Rust` を対象に、code smell、complexity、security hotspot などの server-side issue gate を担当する
...
初期 gate の責務は `Oxlint` / `ESLint` / `Knip` / `rustfmt check` / `SonarScanner + Sonar MCP` / `clippy` に固定する。
...
- `SonarScanner + Sonar MCP` は既存の `Oxlint` / `ESLint` / `Knip` / `clippy` を置き換えず、構造解析系 lint で取り切れない issue を補完する追加層として gate に入れる
...
- implementation lane は Sonar MCP の `search_sonar_issues_in_projects` を直接使って issue を読み、`project == ishibata91_AITranslationEngineJP` かつ `status == OPEN` の issue が残る限り implementing skill に差し戻す
- import graph や cycle の主担当は Sonar に移さず、専用 lint / 静的解析に残す
- project context は `sonar-project.properties` の project key を正本とし、Sonar MCP query でも同じ key を使う
...
- scanner 実行後は implementation lane が Sonar MCP の `search_sonar_issues_in_projects` を使って `project == ishibata91_AITranslationEngineJP` かつ `status == OPEN` issue を取得する
