...
準備の段（`experiment-workflow`）の途中で止めてある。翻訳と判別器を回すのに別マシンが要るため、Mac で進められるところまで進めた状態。

**済んでいること**

- `Skyrim.esm` の抽出、`line` への取り込み、話者 909 人の基底口調の生成。
- 標本の凍結。開発用 598 件・評価用 598 件を `tmp/dialogue-tone-naturalness/dataset/` へ書いた。以後選び直さない。
- 訳す道具（`tmp/dialogue-tone-naturalness/translate`）。空打ちで 598 件すべてに口調指示が付くことを確かめた。
- 達成の線 AUC 0.70 以下の人間承認。

**次の一手**

1. LM Studio（`http://192.168.0.226:1234`）を上げ、`go run ./tmp/dialogue-tone-naturalness/translate -set dev -model <gemma のモデル名> -round 1` で開発用 598 件を訳す。
2. 判別器（形態素解析 + ロジスティック回帰、`criteria.md` の達成条件の 1 行目の数え方）を作り、0 回目の AUC を測って `measurements.csv` へ書く。
3. `criteria_reviewer` に達成条件レビューをかける。通過するまでループの段へ入らない。

**人間の判断が要るもの**

`criteria.md` の人間承認の欄のうち、次が未承認。ループの段へ入る前に埋める。

- 達成条件: 意味の取り違えの件数の線、破損行の件数の線。
- 選択肢: 測る手段・変える手段・標本の取り方の全行。
- 標本: 年配と子供の区分で話者が 17〜20 人しかいないこと、区分の等配分により標本の中で年配と子供が 2/3 を占めることの扱い。

**砂場は commit に入らない。** 道具と凍結した標本は `tmp/` にあり `.gitignore` の 14 行目で除外される。別マシンで続ける場合は道具を作り直すか、`tmp/` を手で運ぶ。凍結した標本は乱数の種 20260727 で再現できる。
...
- `assets/role-speech.tsv`: 一人称と言い回しの傾向。区分ごとに文字列で持つ。
...
- 外部の接続先: `http://192.168.0.226:1234`（LM Studio、LAN 上の別マシン）。翻訳を回す。使うモデルは実験の条件として固定し、回の間で変えない。`hy-mt2-7b` は空応答と記号混入を起こすため使わず、gemma 系を使う。
- 機械: 翻訳は LM Studio の載った別マシン。訳文らしさの判定と意味の保持の測定は RTX4090 と 9800X3D の Windows マシンで回す。測る手段の詳細は `criteria.md` の選択肢の表が持つ。
