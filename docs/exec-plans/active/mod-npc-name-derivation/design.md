...
- 部分形は機械置換辞書の材料であり、翻訳対象ではない。原文 record を持たないので、翻訳対象として数える経路（固有名の一覧、対象 plugin の進捗件数）から外す。
- 判別は `proper_noun` へ足す列 `origin` で行う。列が持つのは抽出由来か派生かの 2 値で、空が抽出由来（翻訳対象）、`derive` が派生（辞書の材料）を表す。既存行は空のまま残るので、既定値を空にした列追加で足りる。どの規則で割ったか（名のみ、二つ名の前部、姓名分割）は絞り込みに要らないので持たない。`category` の値で判別しない。`category` は record 種別を持つ列であり、派生行は record を持たないので値を入れられない。
