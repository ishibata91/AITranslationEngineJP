...
既定値の入れ替えは、既存 seed（`db/migrations/0004_prompt_template.sql:20`）の文面を書き換えて行う。既定値を配るための新規 migration は足さない。未配布で開発中のため、既存 DB に入っている値を残す必要が無い（人間回答）。書き換えた seed を当てるには `db/aitranslation.dev.sqlite3` を消して起動し直す。`SchemaMigrator` は `user_version` で適用済みの本数だけを見るので、DB を残したままでは書き換えが当たらない。
...
**TO-BE**: 7 種を 9 種にする。分割は 2 箇所、割り当ての移動は 1 箇所、改称は 2 箇所である。

反映は既存 seed の書き換えで行う。`db/migrations/0006_record_type_translation.sql:24` の指示文 seed を最初から 9 種で並べ、同 48 行以降の割り当てを新しい対応で書き直す。旧キー（`説明体`・`定型句`）は seed に現れなくなるので、行の追加・付け替え・削除を段階的に行う必要が無い。層1 と同じく、DB を消して作り直すことで反映する。
...
2. 口調を例文で固定する仕組みが無い。`internal/core/personatone/personatone.go:17` のコメントが「一人称・語尾を確定する few-shot 例文は後続の精緻化で足す」と未着手を記録している。台詞は 1 件 1 リクエストで送るため、同じ話者の複数台詞が独立に推論され、説明文だけでは語尾が行ごとに揺れる。`docs/exec-plans/completed/2026-06-20-character-persona-from-dialogue/persona-known-issues.md:54-68`（一人称・語尾が同一話者内で揺れる）で観測済みの症状である。
...
例文は役割語テンプレートとは別の asset ファイル `assets/role-speech-examples.tsv` に置く。キーは役割語と同じ 3 列（種族区分 × 性別 × セル）で、値は「英語原文」「日本語訳文」の 2 列である。同じキーで 2 つの表を引くことになるが、役割語 1 行が一人称・言い回し・英文・訳文を抱えると 1 行が長くなり、人間が中身を見直せなくなる。中身の見直しやすさを優先して分ける。照合規則（ワイルドカードと具体度優先）は役割語と同じにし、`rolespeech` package の `Lookup` を共用する。

行は 57 行にする。内訳は、種族区分 3 × 性別 2 × セル 9 の具体行 54 と、セルを持たない経路のための `adult male *`・`adult female *`・`adult * *` の 3 行である。名指し話者の性別は NPC の Female flag から決まり空にならないため、具体行 54 で全ての名指し話者が一致する。汎用台詞と PC 発話は `Lookup("adult", sex, "")` を呼ぶため（`internal/core/personatone/personatone.go:150-158`）、セル列がワイルドカードの 3 行で受ける。

配線は `rolespeech.Table` を広げて受ける。`Table` が役割語行と例文行の 2 つの行集合を持ち、`Lookup` が同じキーで両方をそれぞれ最も具体的な行へ照合し、返す `Template` に例文のフィールドを足す。ファイルを開くのは composition root（`internal/bootstrap/bootstrap.go`、`cmd/goldcap/main.go`）の責務で、2 つの asset を読んで 1 つの `Table` を組む。この形なら `engine.New`（`internal/engine/engine.go:131`）と `harness.RunConfig`（`internal/harness/run.go:20`、役割語表を受ける field は同 24 行）、`personatone` の関数シグネチャを変えずに済む。

`BuildToneTraits` と `BuildFreeToneTraits` は、引いた例文を `- 例: 英語原文 → 日本語訳文` の 1 行として口調指示へ足す。`provider.Prompt` は `System` と `User` の 2 つしか持たないため、例文は messages の対ではなく system 内の例示として置く。型を変えずに済む。
...
`FillVariables` は削る。呼び出し箇所が無く、`{traits}` の差し込みは `BuildToneDirective` が担っており、残しても口調の雛形が 2 通りに読める状態が続くためである。関数と対応する単体テストを消す。
...
flowchart LR
  D["directive「口調」<br>{traits} を持つ雛形"]
  TT["toneTraits<br>9 セルの性質文"]
  RS["role-speech.tsv<br>一人称・言い回し<br>成人男性と性別不明を含む"]
  EX["role-speech-examples.tsv<br>英語原文 → 日本語訳文<br>54 の具体行 + 3 のワイルドカード行"]
  B["BuildToneDirective"]
  SYS["台詞の system プロンプト"]
  D --> B
  TT --> B
...
AS-IS から消える要素は `prompt_template.persona_template` である。合成に使われないまま読み書きだけが残っている経路を、Go 側と frontend 側の型から消す。TO-BE で増える箱は例文表 `assets/role-speech-examples.tsv` の 1 つで、役割語表と同じキーで引き、口調指示へ例文の 1 行を足す。`role-speech.tsv` は列を変えず、欠落していた行（成人男性のセル別行と成人の性別ワイルドカード行）が埋まる。

### どこまで動かすか

観測点は単体テスト、結合テスト、実画面の 3 つとする。

- 単体テスト（`internal/core/rolespeech`、`internal/core/personatone`）: 例文表（キー 3 列＋英語原文＋日本語訳文）を解析できること。実表を読んで、名指し話者の全区分（種族区分 3 × 性別 2 × セル 9）が役割語と例文の両方で一致を返すこと。汎用台詞経路が性別空でワイルドカード行へ落ち、例文もセル空のワイルドカード行から引けること。口調指示に例文の行が乗ること。
- 単体テスト（`internal/store`）: `TestSeedRecordTypeMasterIntegrity` が 9 種の seed と分割後の box → 指示文の対応で通ること。`record_type_master` の全行が seed 済みの指示文キーを参照すること（孤立参照が無いこと）。
- 結合テスト（`internal/harness`）: 台詞の system プロンプトへ一人称と例文の行が乗ること。叙述文が分割後の指示文（効果説明・操作名・語義）を受け取ること。harness は合成役割語表を使うため（`internal/harness/run.go:87`）、合成の例文表も同じ場所へ足す。合成表を使う以上、実表の行の欠落は検出しない。実表を読んで全区分が一致を返す観測点は `internal/core/rolespeech` 側へ置く。
...
| --- | --- | --- | --- | --- |
| base 翻訳指示文 | 2 文。出力形の指示が構造化出力と重複し、機械置換済み固有名の保持と出力の崩れ方の禁止が無い | `db/migrations/0004_prompt_template.sql:20`、`internal/provider/openai_compatible.go:186`（`response_format` を常に付ける）、`internal/engine/engine.go:292`（機械置換） | 4 段落。役割、固有名の保持、崩れ方の禁止、口調と原文尊重の優先順位 | `db/migrations/0004_prompt_template.sql:20` の seed の文面を書き換える。既定値を配るための新規 migration は足さない。`INSERT OR IGNORE` は既存行を書き換えないため、反映には `db/aitranslation.dev.sqlite3` を消して作り直す（`db/migrate.go` の `Apply` と `tools/extractor/SchemaMigrator.cs` の `Ensure` は `user_version` で適用済みの本数だけを見るので、DB を残すと書き換えた seed が当たらない）。未配布で開発中のため、既存 DB の値を残さない |
| 指示文の粒度 | 7 種。説明体が 13 個、定型句が 5 個の REC:FIELD を受け持つ | `db/migrations/0006_record_type_translation.sql:24`（指示文の seed）と同 48 行以降（割り当ての seed） | 9 種。効果説明と語義を新設し、説明体を物品説明へ、定型句を操作名へ狭める。`RACE:DESC` を世界観断片へ移す | `db/migrations/0006_record_type_translation.sql:24` の指示文 seed を 9 種で書き直し、同 48 行以降の割り当て seed の `directive` 列を新しい対応で書き直す。旧キー（説明体・定型句）は seed から消す。行の追加・付け替え・削除を段階的に行う必要は無く、`record_type_master.directive` の外部キー（同 42 行）に対しても seed 時点で参照先が揃う。反映は層1 と同じく DB を作り直して行う。`internal/engine/engine.go:404` の `directiveLookups` はキーを固定で持たないため（DB の行をそのまま map へ入れる）、種類の増減にコード変更は要らない。指示文キーを名指しする箇所は 3 つで、`internal/engine/engine.go:398` の定数 `directiveTone`、同ファイルの `directiveProperNoun`、`internal/api/app.go:682` の生文字列 `"口調"` である。3 つとも据え置くキーを指すため、9 種化の影響を受けない。既存テスト `internal/store/seed_test.go` の `TestSeedRecordTypeMasterIntegrity` は seed の指示文キーを固定で持つため、同じ変更で書き直す。書き直す箇所は 3 つで、7 種を並べた `wantKeys`（同 25-28 行）を 9 種にし、叙述文 box の許容集合 `narrationDirectives`（同 55 行）を物品説明・効果説明・世界観断片・書物体・日記体にし、定型句 box の単一キー比較（同 83-86 行）を操作名・語義の集合判定にする |
...
| 口調の例文 | 例文を持つ仕組みが無い | `internal/core/personatone/personatone.go:17`（未着手のコメント）、`internal/core/rolespeech/rolespeech.go:36`（5 列で解析） | 例文専用の asset `assets/role-speech-examples.tsv`（キー 3 列＋英語原文＋日本語訳文、具体 54 行＋セルのワイルドカード 3 行）を置き、口調指示へ `- 例:` の行として乗せる | `internal/core/rolespeech` に例文行の集合と解析関数を足し、`Table` が役割語行と例文行の 2 つを持つ形へ広げる。`Template` へ例文のフィールドを足し、`Lookup`（同 62 行）が例文行にも同じ照合（`matchScore`、同 79 行）を掛ける。`ParseRoleSpeech` の 5 列判定は変えない。composition root（`internal/bootstrap/bootstrap.go:30, 62`、`cmd/goldcap/main.go:53, 79`）が 2 ファイルを開いて 1 つの `Table` を組む。`internal/harness/run.go:66` の合成表にも例文の合成入力を足す。`internal/core/personatone` の `BuildToneTraits` と `BuildFreeToneTraits` へ例文行の組み立てを足す。`engine.New`（`internal/engine/engine.go:131`）と `harness.RunConfig`（`internal/harness/run.go:20`、役割語表を受ける field は同 24 行）の型は変えずに済む。`provider.Prompt`（`internal/provider/openai_compatible.go:22`）も変えず、system 内の例示に留める |
...
| 未使用の変数差し込み関数 | `FillVariables` に呼び出し箇所が無い | `internal/core/prompt/prompt.go:16`、`grep` で呼び出し 0 件（定義とテストのみ） | 削る | `internal/core/prompt/prompt.go` から関数と対応する単体テストを消す。差し込みは `BuildToneDirective`（`internal/core/personatone/personatone.go:184`）が担っており、削っても合成経路は変わらない |

## 検討が必要なこと

無い。5 件の論点は人間回答で解消済みで、決定内容は上の実装方針へ織り込んだ。決定の要点は次のとおり。

- 既定値の更新: 既存 seed の文面を書き換え、DB を消して作り直すことで反映する。未配布で開発中のため、既存 DB の値を残さない。
- 出力形の指示: base 翻訳指示文から落とす。出力形は構造化出力の schema が強制する。
- 指示文キーの改称: 改称する。既存 seed を 9 種で書き直し、旧キーは seed から消す。
- 例文の置き場所: 役割語テンプレートとは別ファイルにする。
- 例文の量: 名指し話者の全区分 54 行を埋め、セルを持たない経路のワイルドカード 3 行を足す。
