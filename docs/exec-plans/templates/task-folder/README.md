...
- `design.md`: どう実装し、どう直すかを人間が読んで判断するための説明を持つ。実装方針（AS-IS→TO-BE）と検討が必要なこと。両フロー共通の 1 テンプレート
- `spec.md`: この task の確定仕様（対象集合と、AS-IS / TO-BE を横に並べた期待値表）を持つ。TO-BE の期待値の文はそのまま実テストの test case 名にする。両フロー共通の 1 テンプレート
...
- 仕様を短時間で確認する時は `spec.md` を読む。対象集合と期待値が正。`design.md` と食い違う場合は `spec.md` を優先する
- 設計の理由と変え方は `design.md` を読む。どう実装/どう直すか（実装方針＝AS-IS→TO-BE）が正
