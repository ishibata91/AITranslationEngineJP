...
- task_id: 2026-04-14-agent-name-alignment
- task_catalog_ref: N/A
- parent_phase: N/A

## Request Summary

- primary agent の名前を skill 名に合わせる
- agent file name、`name =`、skill 側参照を同期する
- 直前の agent naming 変更結果を新しい naming へ追従させる

## Decision Basis

- primary mapping は 1 対 1 に整理済みであり、次の論点は agent 名の表現だけであった
- noun-based naming の方が agent role を読み取りやすい
...
- `constraints`: live skill 数は変えない。secondary agent の役割はこの task では再編しない
...
- `skill-modification` は direct agent として `implement` を参照していた
- completed plan 2 件が current naming を記録していた

## Functional Requirements

- `summary`: primary agent 名を noun-based naming へ再統一する
- `in_scope`: `.codex/agents/*.toml`, `.codex/skills/*/agents/openai.yaml`, `.codex/skills/orchestrate/SKILL.md`, `docs/exec-plans/completed/2026-04-14-orchestrate-agent-skill-uniqueness.md`
- `non_functional_requirements`: 命名規則が一貫し、参照切れがない
- `out_of_scope`: product code, docs 正本, secondary agent rename
- `open_questions`: なし
- `required_reading`: `.codex/README.md`, `.codex/workflow.md`, `docs/exec-plans/completed/2026-04-14-orchestrate-agent-skill-uniqueness.md`
...
- `distill`、`design`、`investigate`、`implement`、`tests`、`review`、`diagramming`、`updating-docs`、`skill-modification` の `agents/openai.yaml` を名詞形 agent 名へ更新した
