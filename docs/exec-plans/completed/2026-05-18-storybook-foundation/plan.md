...
- `branch_ready`: `ready`
- `commit_hash`: `bb8fa36f6fe4d2f2367d73f28dc5672a0259820e`
...
- `post_merge_validation`: `git diff --check --cached` pass; `npm --prefix frontend install` completed to install Storybook dependencies; `npm --prefix frontend run build-storybook` pass; `python3 scripts/harness/run.py --suite frontend-local` pass
- `completed_move`: `docs/exec-plans/active/2026-05-18-storybook-foundation/` から `docs/exec-plans/completed/2026-05-18-storybook-foundation/` へ移動済み
- `merge_commit_hash`: `see-local-merge-result-commit`
- `remote_operation`: `not-performed`
...
- `codex/2026-05-18-storybook-foundation` は `master` へ conflict なしで local merge 済みである。
- merge 後検証は通過済みである。
