...
- 追補（merge 後、人間指示「他の乖離を直してくれ」を受領）: 構造は不変だが、`docs/architecture.md` §8 の「extractor は narration/line を書く」という現在状態の記述が実装と乖離していた。人間指示を承認として `architecture.md` を反映へ切り替えた。§3（engine 責務に取込段・固有名フェーズを反映）・§8 現状記述（extractor は `extracted_field` を書き engine の取込段が振り分ける）を修正し、§8 へ本 task の移行 entry（全 REC:FIELD 化・取込段・固有名フェーズ・directive・口調供給の prompt_template→口調 directive 移行・新 Bind）を追加。`docs/architecture.md`・本 plan・changelog の反映は同一 commit に含める（hash は git log 参照）。
- 追補 2（人間指示「er.md は現テーブルと同期しなければならない」を受領）: 当初 `er.md` を据え置きと判断したが撤回。人間の方針は「er.md は `concept-model.md` に基づいて作り実装のため正規化したテーブル設計で、厳密 1 対 1 は不要だが概念モデルと無関係な恣意的 DB はだめ」。`er.md` は当時、概念モデル全体を写した論理 ER（未実装の概念テーブルを含む）で、実 DB と両方向に 8 テーブルずつ乖離していた（実: master_term・prompt_template・persona_character・line_analysis・directive・record_type_master・extracted_field・extracted_info_speaker、未実装: set_phrase・placement・untranslated_fragment・narration_mention・line_mention・line_sequence・speaker_name・faction_name）。`er.md` を実テーブル基準へ作り直した。採用原則を「厳密 1 対 1」から「概念由来＋実装正規化（由来を追える）」へ緩め、テーブルを「概念箱由来／実装・運用（正規化・実現方式）／未実装（概念由来・後続）」の 3 区分で記述。タイトルを「抽出入力」から「中心 DB」へ広げ、`index.md` の説明も合わせた。本 task の 4 テーブルもこの 3 区分に位置づけて反映。
