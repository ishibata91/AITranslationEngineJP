...
- 追補（merge 後、人間指示「他の乖離を直してくれ」を受領）: 構造は不変だが、`docs/architecture.md` §8 の「extractor は narration/line を書く」という現在状態の記述が実装と乖離していた。人間指示を承認として `architecture.md` を反映へ切り替えた。§3（engine 責務に取込段・固有名フェーズを反映）・§8 現状記述（extractor は `extracted_field` を書き engine の取込段が振り分ける）を修正し、§8 へ本 task の移行 entry（全 REC:FIELD 化・取込段・固有名フェーズ・directive・口調供給の prompt_template→口調 directive 移行・新 Bind）を追加。`er.md` は概念モデルそのものではなく、`concept-model.md`（概念の正本）の箱を SQLite テーブルへ 1 対 1 で写した論理 ER（テーブル設計の段）である。その対象は抽出入力の概念 10 箱に限り、config・実現方式・master 辞書を自ら対象外と明記する。新表はその対象外（`directive`・`record_type_master`＝config、`extracted_field`・`extracted_info_speaker`＝実現方式の buffer/staging）か既存箱（`proper_noun`）のため据え置き。`docs/architecture.md`・本 plan・changelog の反映は同一 commit に含める（hash は git log 参照）。
