...
backend レビュー修正 3 と親側再検証が完了したため、5 観点レビューを再実行する。

## 対象指摘

- `state-invariant-002`: 単語翻訳 resume と retry が条件付き状態更新を使っていない。
...
修正内容:
- 単語翻訳 phase の resume は `UpdateJobPhaseRunWhenState` に expected state `paused` を渡す。
- 単語翻訳 phase の retry は `UpdateJobPhaseRunWhenState` に expected state `recoverable_failed` を渡す。
- expected state 不一致相当の `repository.ErrConflict` では provider request を発生させず、phase run が `running` にならないことを test で確認した。
...
- 状態更新は同じ `JOB_PHASE_RUN` を required state 条件付きで更新する。
