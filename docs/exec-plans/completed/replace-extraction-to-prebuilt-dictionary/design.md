...
翻訳Engineは、本文で一致した原語について、事前作成済み翻訳辞書DBに保存された全ての訳語、品詞、Skyrimのカテゴリを読み取り専用で本文翻訳の参考語に使う。`dictionary_sense.meaning` は辞書readerの読取りと検証にだけ使い、本文翻訳のprompt、候補snapshot、翻訳結果UIへ出さない。`inclusion_decision` と `review_stage` は選択条件にしない。

候補の共有型は `internal/model/translation_reference.go` が所有する。`model.PrebuiltDictionaryReference` は辞書readerだけが返す内部候補であり、原語、訳語、品詞、意味、Skyrimのカテゴリを持つ。`model.TranslationReference` は本文用候補であり、原語、訳語、品詞、Skyrimのカテゴリ、出どころを持ち、`meaning`を持たない。Engineは `toTranslationReferences` で事前作成済み辞書の内部候補と翻訳済みの `proper_noun` を本文用候補へ変換する。Engineのconsumer interfaceは本文用候補を受け、`prompt.BodyReference` へ変換する。候補snapshotとAPI DTOは本文用候補と同じ項目だけを持つ。`internal/store` は `internal/core/prompt` に依存せず、`internal/engine` は `internal/store` に依存しない。`internal/model` は両componentが依存を許可される共有層である。

辞書readerは `internal/store` が所有する。`OpenPrebuiltDictionary` は読み取り専用のSQLite接続を作り、実ファイルから `PRAGMA schema_version` を読んで接続先が読取り可能なDBであることを確認する。`OpenPrebuiltDictionary` はschemaのtable、column、全原語と訳語の読取りを検証しない。readerは同じ原語、訳語、品詞、意味、Skyrimのカテゴリを持つ候補だけを一意にして返し、接続のCloseを提供する。本文用候補への変換は、原語、訳語、品詞、Skyrimのカテゴリ、出どころが同じ候補を一つにする。`meaning`だけが異なる候補は本文翻訳の参考語として重複させない。`internal/bootstrap.NewApp` は中心DBとreaderを持つapplication lifecycle closerを返す。`main.go` はapplication lifecycle closerをdeferし、中心DBとreaderを同じ終了処理でCloseする。
...
同じ原語に異なる訳語がある場合は、候補を一つへ絞らない。翻訳指示は各候補を同じ原語の別の参考語として列挙し、AIが本文と品詞、Skyrimのカテゴリから使い分ける。`meaning`だけが異なる候補は、本文用の参考語として一つにする。同期翻訳とbatch翻訳は共通の `composeBodyPrompt` を使うため、同じ本文と同じ参考語を送る。

Engineは、本文用promptを組み立てた時点の本文用候補とpromptのSHA-256を中心DBの `translation_reference_snapshot` へ保存する。候補snapshotは`meaning`を持たない。snapshotの対象plugin keyは、`target_plugin.plugin` と同じplugin名文字列である。同期翻訳はproviderへ送る前にplugin名、kind、row IDへupsertする。batch翻訳は、外部batch送信より前に本文kindのbatch requestだけへ `custom_id` ごとの候補とhashを仮状態で保存する。固有名kindのbatch requestは候補とhashを持たない。外部batch送信の成功後にexternal batch IDを保存して仮状態を送信済みへ更新する。外部batch送信が失敗した場合は仮状態を送信失敗へ更新し、同じ `custom_id` で再送できる状態にする。本文kindの再送は候補とhashも同じ値を使う。送信済みだけを既送として除外する。
...
- `internal/model/translation_reference.go` を追加する。`PrebuiltDictionaryReference` は辞書readerの内部候補として`meaning`を持つ。`TranslationReference`、batch requestへ保存する候補snapshot、`translation_reference_snapshot` へ保存する候補snapshotは、原語、訳語、品詞、Skyrimのカテゴリ、出どころだけを持ち、`meaning`を持たない。`internal/store` と `internal/engine` がこの共有型を使う。
...
- `internal/engine/engine.go` の `Engine`、`New`、`LoadDictionary`、`TranslateUntranslated`、`prepareSource`、`toTranslationReferences`、`composeBodyPrompt` を変更する。参考語専用のreader interfaceと `ValidatePrebuiltDictionary` を追加する。`toTranslationReferences` はreaderの `PrebuiltDictionaryReference` を`meaning`なしの `TranslationReference` へ変換し、本文用の重複除去を行う。`prepareSource` は辞書置換を行わず、事前作成済み翻訳辞書の原語と対象pluginの翻訳済み `proper_noun` だけを候補集合として本文から抽出し、抽出した原語の全参考語を `composeBodyPrompt` へ渡す。`TranslateUntranslated` は `translateProperNouns` より前に検証する。`translationVocabulary` と `authoritativeTerms` の呼び出しを停止する。
...
- `internal/api/app.go` の `TermView` と `ResultView` を変更する。plugin名、kind、row IDで取得した候補snapshotを候補ごとの配列として返す。各 `TermView` は原語、訳語、品詞、Skyrimのカテゴリ、出どころを持ち、`meaning`を持たない。`ResultView` はprompt hashを持つ。保存した候補から再構成したprompt hashが一致しない場合は結果を返さない。snapshotがない既訳再利用、migration前の翻訳済み、固有名の結果は参考語の配列を空にする。辞書訳による本文の再置換を行わない。
- `frontend/src/gateway/translation-gateway.ts` の `TermRow` と `toResultRow` を変更する。backendの `TermView` から原語、訳語、品詞、Skyrimのカテゴリ、出どころだけを画面用の参考語型へ変換し、`meaning`を変換しない。`frontend/src/ui/screens/translation-run/translation-run-view.ts`、`TranslationResultRow.svelte`、`translation-run.fixtures.ts`、`TranslationResultRow.stories.ts`、`TranslationRunScreen.stories.ts` を変更する。Wails生成型を更新する。frontendの参考語型はbackendの `TermView` と同じ原語、訳語、品詞、Skyrimのカテゴリ、出どころを持ち、`meaning`を持たない。候補ごとの参考語を原語でまとめ、同じ原語の複数候補を訳語、品詞、Skyrimのカテゴリ、出どころとともに表示する。参考語の配列が空の結果は参考語を表示しない。Storybookで人間が確認する。
