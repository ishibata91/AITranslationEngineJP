...
- `plan.md`: branch 情報とこの task でやらないことの要点だけ。設計・判断履歴・検証結果は持たない。
- `design.md`: 実装方針（どう実装し、どう変えるかを文章で書く。現状 AS-IS と変更後 TO-BE を対にし、流れ・関係・責務が変わる箇所は図を添える）と、検討が必要なこと。検討事項に未解決の論点が残る間は下流へ進まない。
