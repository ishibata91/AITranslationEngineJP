...
- **訳が文脈で変わるもの**（叙述文・台詞）: 叙述文は文体（物品説明/効果説明/書物体/日記体/世界観断片）が要り、台詞は同じ原文でも話者で口調が変わる（衛兵の「何か用か？」と貴婦人の「何かご用かしら？」）。→ **配置ごとに自前で訳す**。重複排除できない。文中の固有名詞は `固有名` を言及して揃える。
...
- **叙述文の文体**: 物品説明（武器・防具・薬などの説明）/ 効果説明（呪文・付呪・特典の効果。数値と実行時タグを含む）/ 書物体（本）/ 日記体（クエストログ・目標）/ 世界観断片（ロード画面・種族の解説）。重複排除しない代わりに文体が訳し方の手掛かり。
