...
- **種別（固有名）**: 重複排除でまとめるときの同定キー。原文だけでまとめると同綴り異義（`Chest`＝宝箱/胸）を誤って統合するため、レコード種別（WEAP/CONT/LCTN…）を併せ持って区別する。ただし同じ種別内の別物までは分けきれない（既知の弱点 1）。
...
- **言及（e3/e4/e5）**: 叙述文・台詞の文中の固有名詞を `固有名` に結び、その訳に揃える。e3 は叙述文の説明対象（武器の説明文→武器の名前）、e4/e5 は文中の言及。検出は難しい（既知の弱点 2）。
...
- **性質の合成で口調が決まる**: 話者の口調は各素材の `性質` と話者属性の合成結果。合成の優先度・方法（ルールか AI か）は実現方式で固定しない（既知の弱点 3）。

## 既知の弱点（指摘の入口）

| # | 論点 |
|---|---|
| 1 | **固有名の同定単位（決定済み・コスト許容）**: 同綴り異義（`Chest`＝宝箱/胸）は `種別` で粗く分けるが、同種別内の別物は分けきれず誤統合しうる。一貫性（同原語＝同訳）と正確性のトレードオフと整理し、誤統合コストを許容する。FormID 等での完全区別は今回行わない |
| 2 | **言及の実体化（e3/e4/e5）**: 叙述文・台詞の文中の固有名詞をどう検出して固有名へ結ぶかは関係だけ示す。検出方式は実装判断 |
| 3 | **素材の合成優先度**: 種族・勢力・声型・性別・職業傾向の `性質` が衝突する場合の優先度・合成方法は関係だけ示し、実装判断に委ねる |
| 4 | **重複排除の境界**: 「文脈で訳が変わらない」の判定。台詞は話者で変わるので除外したが、汎用 message や一部 RNAM の境界は実装で詰める |
