...
`NPC_PROFILE` は対象プラグイン名、FormID、RecordType などから、抽出スナップショットをまたいで同じ NPC と見なす基準を表す。
`NPC_RECORD` は `TRANSLATION_RECORD` の派生テーブルであり、抽出スナップショット上の NPC 属性を保持して `NPC_PROFILE` を参照する。
...
`PERSONA` は `NPC_PROFILE` と 1:1 で紐づく。
共通ペルソナはジョブ内ペルソナ生成の重複スキップに使うため、同一 NPC プロファイルに共通とジョブ内を同時保持しない。

共通 / ジョブ内、適用範囲、作成経路は `persona_lifecycle`、`persona_scope`、`persona_source` で区別する。
`translation_job_id` はジョブ内データのときだけ設定し、共通データでは空にする。
`PERSONA_FIELD_EVIDENCE` はペルソナ生成根拠の翻訳フィールドを保持する。
`DICTIONARY_ENTRY` は共通辞書とジョブ内辞書を同じテーブルで扱い、`dictionary_lifecycle`、`dictionary_scope`、`dictionary_source` で区別する。

`XTRANSLATOR_TRANSLATION_XML` は辞書項目の取り込み元であり、辞書の所有者ではない。
共通データはジョブをまたいで残し、ジョブ内データは翻訳完了後の flush 対象にできる。
