...
- 単語翻訳対象 REC 集合は、XML 辞書取り込み対象 REC 集合と同一の 13 種別とする（`master-dictionary-REQ-004` と同一集合）。
- 単語翻訳フェーズと XML 辞書取り込みは、同一の単語翻訳対象 REC 判定（`IsTermTarget`）を共有して対象判定を行う。
...
- 単語翻訳対象 REC 13 種別固定と REC 一致条件の詳細仕様差分は、2026-05-31 の人間設計レビュー承認に基づいて反映済みである（`docs/exec-plans/active/term-target-rec-config/detail-spec-diff.md`）。
