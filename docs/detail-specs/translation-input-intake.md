...
- 利用者は、入力データの登録状態、出自、再構築可否、翻訳対象の規模、失敗分類を判断できる。
- 代表例は RecordType、SubrecordType、FormID、EditorID、原文を根拠として示せる。
- 警告、カテゴリ、代表例が未提供の場合は空の値として扱う。

### `translation-input-intake-REQ-003` 入力正本と入力キャッシュを分離する

親要件:
抽出 JSON は入力正本として保持され、入力キャッシュは再構築できる。

仕様:
- 訳文、出力ステータス、出力成果物は入力取り込みの対象外にする。
- 入力キャッシュを削除しても、抽出 JSON 正本から再構築できる。
- 再構築後の翻訳レコード件数、翻訳フィールド件数、カテゴリは再構築前と一致する。
- 再構築は同じ入力データの入力キャッシュ復元として扱う。
- `source_file_missing` は、入力正本が参照不能な場合の失敗分類として扱う。
...
- 失敗または再構築が必要な入力データは、ジョブ設定へ進む拒否理由にする。

### `translation-input-intake-REQ-005` 取り込み状態の差分を判断できる

親要件:
利用者は登録、再構築、再試行、失敗状態を判別できる。

仕様:
- 利用者は xEdit 抽出 JSON の登録、登録済み入力データの選択、入力キャッシュの再構築、失敗時の再試行、ジョブ設定への移行を行える。
- 登録は基盤データ管理が成立し、登録中以外の時だけ成立する。
- 再構築は入力正本の出自情報があり、入力キャッシュ欠落または再構築可能状態の時だけ成立する。
- 再試行は失敗状態と再試行可能な失敗分類の時だけ成立する。
- 未登録、処理中、成功、失敗、利用不可、再試行可能を状態差分として扱う。
- 不正 JSON、入力正本欠落、入力キャッシュ欠落は失敗分類として区別する。
