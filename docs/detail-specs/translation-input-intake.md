...
- 入力取り込みは 1 file を 1 入力データとして扱い、1 入力データを 1 翻訳ジョブ候補として識別する。
- 抽出 JSON を入力正本として扱い、SQLite 側の入力キャッシュは抽出 JSON から再構築できる。
- 翻訳ジョブ作成、翻訳フェーズ実行、AI API 実行、訳文生成、出力成果物生成は扱わない。
...
- 登録対象は xEdit 抽出 JSON である。1 file は 1 入力データとして登録する。
- 入力データは file path、file name、file hash、import timestamp を出自情報として保持する。
- 同一 hash の再登録は拒否し、入力データ件数を増やさない。
...
- 主要操作は xEdit 抽出 JSON の登録、登録済み入力データの選択、取り込み結果の再読込、入力キャッシュの再構築、失敗時の再試行である。
...
- duplicate input、invalid JSON、source file missing、cache missing は error variant として区別する。
...
- 翻訳ジョブ作成、翻訳フェーズ実行、AI API 実行、訳文生成、出力成果物生成。
- app-shell navigation 上の表示名、位置、導線詳細。
- 実装時の一時成果物と実装運用情報。
