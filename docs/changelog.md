...
- 機械置換辞書は本文中の固有名を大文字小文字にかかわらず見つけ、登録済みの同じ訳語へ置換する。
- 言及検出は機械置換辞書と同じ照合規則を使う。語境界、最長一致、横断辞書を plugin 内訳語より先に採る順序は維持する。
- 本文に大文字小文字だけが異なる同じ固有名が複数ある場合、置換内訳と検出結果は登録済みの固有名1件にまとめる。
