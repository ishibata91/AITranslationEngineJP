...
## 2026-07-05 辞書に無い漏れ語の候補検出方式の確定（LLM 不使用）

### 変更

- `internal/core/mention/candidate.go`: 未知固有名候補の検出器 `CandidateDetector` を追加した（言及検出の未知語側）。文頭以外の大文字始まりヒューリスティック、句の結合（接続語 of/the/&・小文字接頭の姓・所有格）、既知語の彫り出し、用法分布（小文字用法）による文頭語の選別、称号・種別語の派生分割、一般語 stoplist の選別を組み合わせた決定的な純粋ルール。単体テストはカバレッジ 100%。
- `internal/core/mention/prose.go`: prose の固有表現抽出と品詞解析の adapter `ProseAnalyzer` を追加した。曖昧位置 1 語の救済と、命令形のクエスト目標行（Kill Vittoria Vici）の識別に使う。差し替え境界は `TextAnalyzer`。
- `cmd/poc-missing-term`: 評価ハーネスを追加した。既知語 held-out 方式（言及付き本文をサンプリングし、言及語を辞書から隠して正解にする）で recall・代理精度・重複・決定性を測る。
- `.go-arch-lint.yml`: `poc-missing-term` を component 登録した。`mention` は `dictionary` を import せず、stoplist を使う側の小さい境界（`mention.Stoplist` interface）で受ける。
- `docs/known-issues.md`: 1 番の「拾い上げ方式は未確定」を「候補検出方式は確定・実装済み。残るのは製品統合と訳語供給」へ更新した。2 番へ本文中の記法保護（タグ・制御文字の翻訳時保持）を独立課題として追記した。`docs/roadmap.md`: 1 番の内訳へ漏れ語候補検出の製品統合を追記した。

### 判断

- LLM（AI 抽出）は使わない方針を確定した。決定性（同一入力に同一出力）を素直に満たすため、および翻訳自体で使う AI provider への追加問い合わせを増やさないため。held-out（Dawnguard.esm・Dragonborn.esm・inigo.esp、開発中は一切参照しない）で recall 95.4%・重複 0・3 回実行のブレ 0 を確認した。
- 精度の当初閾値 90% は 50%（人手ラベリングによる真の精度）へ調整した。精度を上げる選別（見出し行の選別・機能語先頭の句全体抑止の拡大）はいずれも recall を 95% 未満へ下げることを dev の実測で確認した。候補は人間レビューを経て辞書へ入る前提のため、回収不能な見逃し（recall）を硬い制約とし、レビューで捨てられる誤検出（精度）を従とした。実測は dev 約 63%・held-out 約 54%（分析は completed plan の results.md）。
- 評価の正解ラベルは既知語 held-out 方式で機械生成した。人手ラベリング不要で、開発用（Skyrim.esm）と held-out を別 plugin に分ける汎化検証とそのまま接続できるため。正解側の選別（記号だけの辞書行・文形の辞書行・タグ内言及・本文全体が名前の行の除外）はハーネス側の責務にした。
- 用語特定と訳語供給は別軸のまま保つ（本 task は候補検出のみ。訳語供給と製品統合は known-issues 1 番の後続）。
