...
- Gemini・xAI の batch mode（`gemini-xai-batch-translation` task で設計中）は、個別行の翻訳がコンテンツフィルタ（暴力・差別表現等の判定）や不正なリクエストで、再送しても直らない形で恒久的に失敗する場合がある。失敗理由を保持する設計は決めたが、失敗した行を人間が見て手で直す編集手段は課題5（結果画面の編集機能未実装）に依存し、現状は存在しない。
...
`plugin`・`form_id`・原文が既存訳（公式日本語版ローカライズ文字列等、別途保持する参照訳）と完全一致するレコードでも、これを検出して既存訳へ機械的に置き換える機構が無い。現状は完全一致であっても AI 翻訳へ回り、既存訳を参照しない。

## 8. 本文中の実行時タグを保護する機構の未実装

叙述文・台詞の原文に含まれる `<Alias=...>` 等、ゲーム実行時に差し込まれるタグを、AI 翻訳の前後で破壊・改変から守るガード機構が無い。AI 翻訳がタグの文字列を書き換えたり削除したりしても、それを検出する手段が無い。
