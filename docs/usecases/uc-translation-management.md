...
- アクター: Skyrim Mod 翻訳者
- 目的: Skyrim Mod 翻訳者が、完了済みの現在段階から次の翻訳段階または翻訳完了確認へ進める。

#### 2. 条件

- 翻訳実行シェルで選択ジョブを開いている。
- 現在段階が完了している。
- 次の作業フッターが次へ進める状態を表示している。

#### 3. 主シナリオ

1. Skyrim Mod 翻訳者が、次のフェーズへ進む操作を要求する。
2. システムが、現在段階、完了状態、次段階条件を検証する。
3. システムが、選択ジョブの現在段階を次の翻訳段階へ切り替える。
4. システムが、次の翻訳段階の状態、進行状況、処理対象一覧を表示する。

#### 4. 代替シナリオ

##### A1. 本文翻訳完了後に翻訳完了確認へ進む

- 発生条件: 現在段階が本文翻訳で、本文翻訳が完了している。
- 流れ:
  1. Skyrim Mod 翻訳者が、次のフェーズへ進む操作を要求する。
  2. システムが、翻訳完了確認画面を表示する。
- 結果: Skyrim Mod 翻訳者は、本文翻訳後の原文と訳文を確認できる。

#### 5. 例外シナリオ

##### E1. 次へ進める条件が不足する

- 発生条件: 現在段階が未完了、失敗、キャンセル済み、または次段階条件を満たさない。
- システム応答: システムは、次へ進む操作を無効にし、確認が必要な理由を表示する。
- 業務状態: 現在段階と選択ジョブは変更されない。
- 再実行条件: Skyrim Mod 翻訳者が、現在段階を完了させる、または不足条件を解消して再実行できる。

#### 6. 完了条件

- 成功条件: 次の翻訳段階または翻訳完了確認が表示される。
- 失敗条件: 現在段階の画面が維持され、次へ進めない理由が表示される。
- 不変条件: 次へ進む操作は、翻訳結果を再生成しない。
