...
- 触る範囲を 1 文脈で書く。backend、frontend、統合境界を別 agent に渡さない。
...
- backend を触ったときだけ backend 系の検証 suite を実行する。
- frontend を触ったときだけ frontend 系の検証 suite を実行する。
...
- 触った範囲だけ動かす。
    - backend を触ったら backend 系 suite
    - frontend を触ったら `python3 scripts/harness/run.py --suite frontend-local`
