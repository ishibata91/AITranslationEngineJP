...
- 承認済み `spec.md` の対象集合と期待値表が固定されている。`design.md` と `spec.md` が食い違う場合は `spec.md` を実装根拠にする。
...
- `spec.md` の TO-BE の期待値それぞれに、対応する実テストが書かれているか、対応を置けない理由が固定されている。
...
- `spec.md` の TO-BE の期待値の文を、対応する test case の名前としてそのまま使う。人間が実行結果の一覧を読んだだけで、どの期待値が確かめられたか分かる状態にする。
- 単体テストの守備範囲は次に限定:
    - AI サービスの key 保存系
    - プロンプト構築 logic
    - 独立した純粋 class 系
...
- `spec.md` の期待値表と実テストを照合する。TO-BE の期待値 1 行ごとに、対応するテストの file と test case 名を `spec.md` の「対応する実テスト」列へ書き、作業結果としても返す。
- 対応する実テストを置けなかった期待値は、実装が仕様より狭い可能性を示すため、停止理由または残課題として人間へ上げる。
- 実装した対象集合が `spec.md` の対象集合より狭い、または広いと分かった場合は、`spec.md` を正とし、差分を人間へ上げる。
...
- 期待値と実テストの対応: `spec.md` の TO-BE の期待値ごとのテストの file と test case 名、対応を置けなかった期待値とその理由。
...
- `spec.md` の対象集合と実装した対象集合が食い違い、承認済み仕様の変更なしに解消できない。
