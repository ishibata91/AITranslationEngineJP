...
- frontend 実装規約: [implement-frontend/SKILL.md](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/implement-frontend/SKILL.md) とする。
- 統合境界実装規約: [implement-integration/SKILL.md](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/implement-integration/SKILL.md) とする。
...
- リファクタ調査記録がある場合は、構造品質調査とテスト品質調査を実装範囲の根拠にする
...
- 各 引き継ぎ の想定変更ファイル数と変更行数を見積もり、`規模判定条件` の `通常`／`注意`／`分割必須`／`強制停止` に従った。
- `境界規約` の use case 別分割原則を守り、層をまたぐ場合も受け入れユースケース 完了条件 で完了判定できる。
- `引き継ぎ分割規約` に従って backend、frontend、統合境界の分割と UI 順序を満たした。
- frontend 引き継ぎ がある場合は、承認済み `screen-design-diff.<screen-id>.md` を根拠参照に含めた。
- 単体テスト 引き継ぎには、期待結果の元ネタになる承認済み詳細仕様差分または詳細仕様正本を含めた。
- `並列実行規約` に従って `実行グループ`、`ready_wave`、`並列可能対象`、`並列不可理由` を書いた。
- 並列可能な 引き継ぎ の 承認済み実装範囲、shared 境界、検証 担当者 が重なっていない。
- `secret 境界規約` を満たして書いた。
