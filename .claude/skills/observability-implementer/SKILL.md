...
- 影響範囲修正は、既存ログ追加が直接壊したログ payload、出力先、禁止項目確認だけに限定する。
...
- 影響範囲修正: 既存ログ追加が直接壊したログ payload、出力先、禁止項目確認を修正した場合に、対象、理由、変更結果を返す。

## 作業を完了できる条件

- 承認済み実装範囲 または観測ログの影響範囲修正 の成果だけが返却されている。
- 追加ログ または 追加しない理由 が根拠参照付きで返っている。
- 禁止ログ確認 が返っている。
- 観測ログ仕様に従った payload、出力先、禁止項目の確認が返っている。
- 変更ファイル が返っている。
- 検証未実行理由 または検証結果が返っている。
- 影響範囲修正 がある場合は、既存ログ追加が直接壊したログ payload、出力先、禁止項目確認の対象、理由、変更結果が返っている。
...
- 承認済み実装範囲 外の観測ログ変更が必要な場合は停止する。
- 既存ログ追加が直接壊したログ payload、出力先、禁止項目確認だけは、影響範囲修正 として直す。
