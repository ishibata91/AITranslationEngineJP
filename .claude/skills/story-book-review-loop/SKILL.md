...
description: 人間が別セッションで起動した Claude 本体が Storybook 人間レビューを反復し、ブラウザコメント、frontend 修正、変更された画面仕様、設計整合入力を管理する作業プロトコル。
---
# Storybook Review Loop

## 目的

`story-book-review-loop` は、人間が別セッションで起動した Claude 本体が Storybook 人間レビューを反復し、frontend 修正と確定した画面仕様を揃える作業プロトコルである。
frontend を実装する範囲は `implement-frontend` と同等の frontend 実装境界を引き継ぐ。
作業計画フォルダの `storybook-review-loop.md` は、Storybook レビューループの結果を記録する証跡である。

## 対応ロール

- 人間が別セッションで起動した Claude 本体が使う。
- 呼び出し元は人間とする。
- 返却先は人間とする。
...
- 人間コメント: ブラウザで受けたコメント本文、対象 story、対象 selector、frame URL、marker screenshot。
- 人間承認状態: Storybook レビューの承認、差し戻し、追加質問。

## 作業前に読む正本

- ブラウザ操作は `mcp__Claude_in_Chrome` ツール群を MCP ツールとして実行する。
- frontend 実装境界は [implement-frontend](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/implement-frontend/SKILL.md) に従う。
...
- ブラウザで Storybook を開き、人間コメントをレビュー入力として扱う。
...
- `story-book-review-loop` は `screen-design-diff.<screen-id>.md` を作成または更新しない。画面仕様が変わった場合は設計整合入力として返すに留める。
