...
| `画面再現確認` | 画面設計のセレクタに従い、人間観測記録のユーザー操作を `mcp__Claude_in_Chrome` ツールで再現する。 |
...
- 観測事実と仮説を分ける。
- 人間観測記録の画面操作は、原因仮説を固定する前に `mcp__Claude_in_Chrome` ツールで再現確認する。
- 画面再現確認では、呼び出し元から渡された Wails process または接続先へアクセスする。
- 画面再現確認では、画面設計書の selector または `data-testid` に従ってユーザー操作を再現する。
- 画面再現確認では、`mcp__Claude_in_Chrome` ツール群を MCP ツールとして実行する。
...
- 画面再現確認: `mcp__Claude_in_Chrome` ツールで再現に使った再現手順、操作結果、画面状態、証跡参照を返す。
- 確定原因: 観測で確定した原因を返す。
...
- 画面設計の selector に従ったユーザー操作を `mcp__Claude_in_Chrome` ツールで再現確認できない場合は停止する。
