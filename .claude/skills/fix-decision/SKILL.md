---
name: fix-decision
description: 修正レーンで、観測記録から仮説、観測ログ検証、確定原因、修正方針、禁止修正を固定する作業プロトコル。
---
# Fix Decision
...
- 修正レーンの進行境界は [fix-lane](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/fix-lane/SKILL.md) に従う。
...
| `画面再現確認` | 画面設計のセレクタに従い、人間観測記録のユーザー操作を `agent-browser` で再現する。 |
...
- 観測事実と仮説を分ける。
- 人間観測記録の画面操作は、原因仮説を固定する前に `agent-browser` で再現確認する。
- 画面再現確認では、呼び出し元から渡された Wails process または接続先へアクセスする。
- 画面再現確認では、画面設計書の selector または `data-testid` に従ってユーザー操作を再現する。
- 画面再現確認では、`agent-browser` の全 command を `.claude/settings.json` の許可済みコマンドとして実行する。
...
- 画面設計の selector に従ったユーザー操作を `agent-browser` で再現確認できない場合は停止する。
