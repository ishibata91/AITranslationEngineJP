...
`designer` agent と Claude 本体が、詳細仕様差分、画面設計差分、implementation-scope を task 内成果物として固定する時の、人間可読な実行説明の正本として使う。

作業流れの次実行判断、作業計画フォルダ進行管理、人間向け実装引き継ぎの返却は呼び出し元が担当する。
プロダクトコードとプロダクトテストは変更しない。

## 対応ロール

- `designer` が使う。
- 呼び出し元は `design-module`、`storybook-module`（画面設計差し戻し時）、または `designer` agent を Task ツールで起動した上位 agent とする。
- 返却先は人間レビューまたは呼び出し元とする。
- 担当成果物は `design-bundle` の出力規約で固定する。

## 呼び出し元から渡される情報

- 呼び出し元: `design-module`、`storybook-module`、または `designer` を起動した上位 agent を受け取る。
...
- 詳細仕様差分: [detail-spec-design](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/detail-spec-design/SKILL.md) に従う。
- implementation 対象範囲: [implementation-scope](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/implementation-scope/SKILL.md) に従う。
...
詳細仕様差分は [detail-spec-design](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/detail-spec-design/SKILL.md) に従って作る。
...
implementation-scope は [implementation-scope](/Users/iorishibata/Repositories/AITranslationEngineJP/.claude/skills/implementation-scope/SKILL.md) に従って分割する。
...
- 引き継ぎまたは停止理由: 呼び出し元へ戻す理由または停止理由を返す。
...
- 完了判断材料として、呼び出し元が次の作業流れ実行判断、人間レビュー、人間向け実装引き継ぎを判断できる情報が返っている。
