...
description: "Storybook 表示実装（svelte 表示コンポーネント、props、style、story、fixture）、Storybook 人間レビューループ、合意済み frontend 保護を兼ねるモジュール。表示実装と表示修正の作業本体は`fork`へ委譲する。承認済みの story と svelte コンポーネントが画面の正本になる。state / API / Wails bridge / ルーティング / 副作用 などの frontend ロジックは implementation-workflow が扱う。実装方針を越える画面変更は design-workflow へ差し戻す。TRIGGER when: 画面の表示変更がある（layout、文言、style、表示構造のいずれか）。SKIP when: 表示を変えない、または state / API / Wails / ルーティング などの frontend ロジックだけの変更。"
---
# Storybook Module
...
frontend ロジック層（state、API、Wails bridge、ルーティング、副作用、フォーム validation）は `implementation-workflow` が扱う。
...
「扱わない」列の変更が必要な場合は `implementation-workflow` へ渡す。
...
- `design-workflow` の出口（`design.md` の実装方針、画面表示の変更が有り）が承認済み。
...
- 実装方針を越える画面変更が要る場合は、`design-workflow` へ差し戻して実装方針が更新済み。
...
- 実装対象外: state、API 呼び出し、Wails bridge、ルーティング、副作用、フォーム validation のロジック。列挙した frontend ロジックは `implementation-workflow` で扱う。
...
- 実装方針を越える画面変更（新規画面、方針外要素の追加など）は行わない。越える必要がある場合は `design-workflow` へ差し戻して実装方針を見直す。
- 表示範囲を越える変更（state、API、Wails bridge、ルーティング、副作用）が必要になった場合は `implementation-workflow` へ戻す。
...
- `合意済み frontend 保護` を固定するまで、`implementation-workflow` へ進めない。
...
- backend 実装、frontend ロジック実装、統合境界実装は本モジュールでは扱わない。backend 実装、frontend ロジック実装、統合境界実装は `implementation-workflow` で扱う。
...
- 設計差し戻し入力: 実装方針を越えると判断した画面変更、対象画面、根拠、`design-workflow` への差し戻し要否。
- 合意済み frontend 保護: 承認済み画面、表示規則、確認済み Storybook 状態、変更禁止範囲。
- 検証結果: `build-storybook` と `npm run test:frontend` の通過状態または未実行理由。
- 表示範囲外の残課題: `implementation-workflow` へ渡す frontend ロジック変更点（state、API、Wails bridge、ルーティング、副作用、フォーム validation のうち、本モジュールで明らかになった要件）。
- 停止判定: 停止理由、不足項目、戻し先。
...
- 実装方針を越える画面変更が必要だが、`design-workflow` への差し戻しを固定できない。
