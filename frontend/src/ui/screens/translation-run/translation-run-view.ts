...
// 翻訳の配送方式。sync=同期（OpenAI 互換・LM Studio）、openai=OpenAI の batch、xai=xAI の batch。
// 選択で、モデル取得先と起動操作（実行 / 状態確認・取り込み・送信）を出し分ける。
export type TranslationProvider = "sync" | "openai" | "xai"

// batch の進行段。proper=固有名 batch、body=本文 batch、done=完了。
// 固有名 → 本文 → 完了 の順に進む。状態確認で得た進行段をパネルの見出しに出す。
export type BatchStage = "proper" | "body" | "done"

// OpenAI または xAI の batch 進行状況（状態確認で取得）。現段 batch の件数と、取り込める完了段があるか。
...
// 本文翻訳時に送った参考語の候補 1 件。
// 同じ原語には複数の意味候補を持てる。本文は置換せず、候補だけを翻訳指示と結果画面へ残す。
export interface TranslationReference {
  // 原語。本文の最長一致で抽出した語。
  source: string
  // 参考訳。本文へ置換しない。
  dest: string
  // 品詞。mod 固有名だけの候補は空文字にする。
  partOfSpeech: string
  // Skyrim のカテゴリ。mod 固有名だけの候補は空文字にする。
  skyrimCategory: string
  // 参考訳の出どころ。事前作成済み辞書または翻訳済み mod 固有名。
  origin: string
}
...
export interface NarrationResultRow {
  ...
  // 話者を解決できた台詞だけ持つ。
  persona?: PersonaMeta
  // この本文を送る時点で参照した候補。置換が無い行は空または未設定。
  // 畳んだ行には原語数チップ、展開で原語ごとの全候補を出す。
  terms?: TranslationReference[]
