...
export class TranslationJobSetupUseCase {
  ...
  acknowledgeCredentialConfigured(phaseId: TranslationJobSetupPhaseId): void {
    void this.persistPhaseCredential(phaseId)
  }
  ...
  async savePhaseCredential(
    ...
    await this.persistPhaseCredential(phaseId, apiKey)
  }

  private async persistPhaseCredential(
    phaseId: TranslationJobSetupPhaseId,
    apiKey?: string
  ): Promise<void> {
    const state = this.store.snapshot()
    ...
    if (!this.gateway?.saveTranslationJobSetupCredential) {
      this.store.update((draft) => {
        draft.errorMessage =
          "APIキーの保存経路が未接続です。画面を更新してから再実行してください。"
      })
      throw new Error("translation-job-setup credential save gateway is not connected")
    }
    ...
    try {
      ...
      const savedCredential =
        await this.gateway.saveTranslationJobSetupCredential({
          provider: selection.provider,
          credentialRef: requestCredentialRef,
          apiKey: apiKey?.trim() ?? ""
        })
      ...
      this.store.update((draft) => {
        ...
        const latestSelection = findPhaseSelection(draft, phaseId)
        if (!latestSelection) {
          return
        }

        draft.options.credentialRefs = draft.options.credentialRefs.map((credential) =>
          credential.provider === savedCredential.provider
            ? {
                ...credential,
                credentialRef: savedCredential.credentialRef,
                isConfigured: savedCredential.isConfigured,
                isMissingSecret: savedCredential.isMissingSecret
              }
            : credential
        )

        const capability = findCapability(draft.options, latestSelection.provider)
        const credential = resolveCredentialReference(
          draft.options,
          latestSelection.provider,
          savedCredential.credentialRef
        )
        ...
        replacePhaseSelection(draft, {
          ...
          credentialRef: normalizePhaseCredentialRef(
            draft.options,
            latestSelection.provider,
            credential?.credentialRef ?? savedCredential.credentialRef
          ),
          credentialStatus: toCredentialStatus(capability, credential),
          modelListSourceToken: ""
        })
        replaceModelList(draft, {
          ...
          credentialStatus: savedCredential.isConfigured ? "configured" : "missing",
          requestToken: "",
          sourceToken: "",
          status: "not_updated",
          models: []
